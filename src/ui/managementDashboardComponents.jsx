...
function ManagerAssignedTasksV2({state,setState,currentUser,onAssignTask}) {
  ...
  return <div>
    ...
    <div style={{display:"grid",gap:8}}>{rows.map(task=><div key={task.id} className="manager-assigned-row" style={{background:"#fff",border:`1.5px solid ${delayLvl(task.dueDate,task.status)?"#FCA5A5":"#E2E8F0"}`,borderRadius:13,padding:"12px 14px",display:"flex",alignItems:"flex-start",gap:10,minWidth:0}}>
      ...
      <select className="manager-assigned-status" onClick={event=>event.stopPropagation()} style={{...iStyle,width:150,background:"#F8FAFC",fontSize:11}} value={task.status||"Bekliyor"} onChange={event=>updateTask(task.id,{status:event.target.value})}>{STATUSES.map(status=><option key={status}>{status}</option>)}</select>
