...
function ManagerAssignedTasksV2({state,setState,currentUser,onAssignTask}) {
  ...
  return <div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(135px,1fr))",gap:10,marginBottom:14}}>
      {[["Toplam Atama",tasks.length,"var(--accent)"],["Aktif",active.length,"var(--warning)"],["Geciken",active.filter(task=>delayLvl(task.dueDate,task.status)).length,"var(--danger)"],["Tamamlanan",tasks.filter(task=>task.status==="Tamamlandı").length,"var(--success)"]].map(([label,value,color])=><div key={label} className="soft-panel" style={{borderTop:`3px solid ${color}`,padding:13}}><div style={{fontSize:10,color:"var(--muted)",fontWeight:800}}>{label}</div><b style={{fontSize:23,color}}>{value}</b></div>)}
    </div>
    <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",flexWrap:"wrap",marginBottom:12}}>
      <div style={{fontSize:12,color:"var(--muted)",fontWeight:800}}>Geciken görevler listede üstte görünür.</div>
