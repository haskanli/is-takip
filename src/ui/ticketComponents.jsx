...
const PROJECT_CHIP_COLORS=["#0b8a94","#277a64","#9a6b22","#7a6b2f","#3d7684","#8a5f49","#5f7f75","#6c7463"];
const hashText=(value="")=>String(value||"").split("").reduce((total,char)=>((total*31)+char.charCodeAt(0))>>>0,0);
const projectChipColor=(project)=>PROJECT_CHIP_COLORS[hashText(project?.id||project?.name||"project")%PROJECT_CHIP_COLORS.length];
const projectChipStyle=(project)=>({"--project-chip-color":projectChipColor(project)});
          ...
          {rows.map(({ ticket, project, projectId }) => {
            ...
            return (
              <tr key={`${projectId || project?.id || "project"}-${ticket.id}`} onClick={() => onOpen?.({ ticket, project, projectId })}>
                ...
                {showProject && <td><span className="ui-chip project-chip" style={projectChipStyle(project)}>{project?.name || "Proje yok"}</span></td>}
                <td>
                  {customerMode ? (
...
export function TicketsPage({
  ...
  return <div className="theme-work-page tickets-theme-page" style={{padding:"clamp(16px,4vw,28px)",flex:1,overflow:"auto"}}>
    ...
    {ticketView==="list"?<TicketListTable rows={sortedFiltered} people={state.people} customerMode={customerMode} showProject={!customerMode} onOpen={({ticket,projectId})=>setModal({type:"detail",projectId,data:ticket})} onStatusChange={(projectId,id,status)=>update(projectId,id,{status})} onDelete={(projectId,id)=>{if(confirm("Ticket silinsin mi?"))remove(projectId,id);}} canDelete={(ticket)=>isAdmin||ticket.author===currentUser.name}/>:<div className="mobile-ticket-list-grid ticket-card-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(360px,100%),1fr))",gap:10}}>{sortedFiltered.map(({ticket,project,projectId})=>{const age=Math.max(0,daysDiff(ticket.ts));const assignee=state.people.find(p=>p.id===ticket.assignedTo);return <div className="compact-list-card ticket-mobile-card ticket-card" key={`${projectId}-${ticket.id}`} onClick={()=>setModal({type:"detail",projectId,data:ticket})} style={{borderTop:`3px solid ${projectChipColor(project)}`,padding:"14px 15px",cursor:"pointer"}}>
      <div className="ticket-card-meta-row" style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><span className="ui-chip ui-chip-accent ticket-number-chip">{ticketNumber(ticket)}</span>{!customerMode&&<span className="ui-chip project-chip ticket-project-chip" style={projectChipStyle(project)}>{project?.name||"Proje yok"}</span>}<b className="text-wrap-safe ticket-card-title" style={{fontSize:13,color:"var(--text, #112327)"}}>{ticket.title}</b>{customerMode?<span className={`ui-chip ticket-status-chip ${ticketToneClass(ticket.status||"A\u00e7\u0131k")}`}>{ticket.status||"A\u00e7\u0131k"}</span>:<select className="ticket-status-select" value={ticket.status||"A\u00e7\u0131k"} onClick={event=>event.stopPropagation()} onChange={event=>update(projectId,ticket.id,{status:event.target.value})} style={{fontSize:10,border:"1px solid var(--border, #cfe0e3)",borderRadius:9,padding:"4px 7px",background:"rgb(255 255 255 / 78%)",color:"var(--text, #112327)"}}>{TICKET_STATUSES.map(status=><option key={status}>{status}</option>)}</select>}<span className={`ui-chip ticket-age-chip ${age>=7?"ui-chip-danger":"ui-chip-muted"}`} style={{marginLeft:"auto",fontSize:11,fontWeight:800}}>{age}g</span></div>
...
function RecurringProblemsPanel({entries=[]}) {
  ...
  if(!repeated.length)return <div className="soft-panel empty-panel ui-section-card"><b>Tekrar eden problem bulunmuyor.</b><div style={{fontSize:10,marginTop:5}}>Problem kodu veya benzer başlıkla eşleşen en az iki ticket burada görünür.</div></div>;
  return <div className="soft-panel ui-section-card" style={{padding:16,marginBottom:13}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:9}}>{repeated.map(group=><div className="compact-list-card ui-section-card" key={`${group.label}-${group.items.length}`} style={{padding:"12px 13px",fontSize:10,color:"var(--muted, #5b6f74)"}}><div style={{display:"flex",justifyContent:"space-between",gap:8}}><b className="text-wrap-safe" style={{fontSize:12,color:"var(--text, #112327)"}}>{group.label}</b><span className="ui-chip ui-chip-warning">{group.items.length} kez</span></div><div style={{marginTop:8}}>{group.effort.toFixed(1)} saat efor · {[...group.projects].length} proje</div><span style={{display:"block",color:"var(--muted, #5b6f74)",marginTop:4}}>{group.explicit?"Ortak problem koduyla eşleşti":"Başlık benzerliğine göre aday"}</span></div>)}</div>
  </div>;
}
function TicketForm({ project, onSave, onClose, types, prios, people, customerMode=false }) {
  ...
  return <div>
    ...
    <div className="soft-panel ui-section-card ticket-detail-card" style={{padding:12,marginBottom:12}}>
      <div style={{fontSize:11,fontWeight:900,color:"var(--muted, #5b6f74)",marginBottom:9}}>Standart Talep Bilgileri</div>
      <div className="ticket-form-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10}}>
    ...
    <div className="ticket-form-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:11 }}>
    ...
    <div className="ticket-form-grid" style={{display:"grid",gridTemplateColumns:customerMode?"1fr":"1fr 1fr",gap:11}}>
    ...
    {!customerMode&&<div className="ticket-form-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:11}}>
...
function StepListEditor({title,items=[],onAdd,onRemove,onChange}) {
  return <div className="ticket-step-editor" style={{marginBottom:12}}>
    ...
    <div style={{display:"grid",gap:7}}>
      {(items.length?items:[""]).map((item,index)=><div className="ticket-step-row" key={index} style={{display:"grid",gridTemplateColumns:"auto minmax(0,1fr) auto",gap:7,alignItems:"center"}}>
        <span className="ticket-step-index" style={{width:22,height:22,borderRadius:8,display:"grid",placeItems:"center",background:"var(--surface-soft, #f6fafb)",color:"var(--muted, #5b6f74)",fontSize:10,fontWeight:900}}>{index+1}</span>
...
function TicketDetail({ ticket, customerMode=false, canEdit, onClose, onUpdate, onResend, types, prios, people=[] }) {
  ...
  if(customerMode){
    ...
    return <Modal title={`${ticketNumber(ticket)} · ${ticket.title}`} onClose={onClose} wide>
      <div>
        {ticket.description&&<div className="ticket-detail-description" style={{fontSize:13,color:"var(--muted)",lineHeight:1.6,marginBottom:16}}>{ticket.description}</div>}
        <div className="ui-detail-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:9,marginBottom:16}}>
          <div className="ui-mini-stat" style={{background:"var(--surface-soft)",border:"1px solid var(--border)",borderRadius:12,padding:12}}><div style={{fontSize:10,color:"var(--muted)",fontWeight:850,textTransform:"uppercase"}}>Durum</div><div style={{fontSize:15,fontWeight:900,color:"var(--text)",marginTop:4}}>{ticket.status||"Açık"}</div></div>
          <div className="ui-mini-stat" style={{background:"var(--surface-soft)",border:"1px solid var(--border)",borderRadius:12,padding:12}}><div style={{fontSize:10,color:"var(--muted)",fontWeight:850,textTransform:"uppercase"}}>Öncelik</div><div style={{fontSize:15,fontWeight:900,color:"var(--text)",marginTop:4}}>{ticket.priority||"-"}</div></div>
          <div className="ui-mini-stat" style={{background:"var(--surface-soft)",border:"1px solid var(--border)",borderRadius:12,padding:12}}><div style={{fontSize:10,color:"var(--muted)",fontWeight:850,textTransform:"uppercase"}}>Açılış</div><div style={{fontSize:15,fontWeight:900,color:"var(--text)",marginTop:4}}>{ticket.ts?new Date(ticket.ts).toLocaleDateString("tr-TR"):"-"}</div></div>
        </div>
        {(ticket.jiraStatus||ticket.jiraKey||ticket.jiraSummary||ticket.jiraLink)&&<div className="ticket-detail-card" style={{border:"1.5px solid var(--border)",borderRadius:12,padding:14,background:"var(--surface-soft)",marginBottom:16}}>
          ...
          {ticket.jiraLink&&<a className="ui-link-button" href={ticket.jiraLink} target="_blank" rel="noreferrer" style={{display:"inline-block",background:"var(--accent)",color:"#fff",borderRadius:8,padding:"6px 11px",fontSize:12,fontWeight:700,textDecoration:"none"}}>Jira'da Aç</a>}
        </div>}
        <div className="ticket-detail-card is-accent" style={{border:"1px solid color-mix(in srgb, var(--accent) 22%, var(--border))",borderRadius:12,padding:13,background:"var(--accent-ink)",marginBottom:16}}>
        ...
        <div className="ticket-detail-card" style={{border:"1px solid var(--border)",borderRadius:12,padding:13,background:"var(--surface-soft)",marginBottom:16}}>
          <div style={{fontWeight:800,fontSize:12,marginBottom:9}}>Ticket Geçmişi</div>
          <div style={{display:"grid",gap:7}}>{[...(ticket.history||[])].reverse().map(entry=><div className="ticket-history-row" key={entry.id} style={{fontSize:10,color:"var(--muted)",display:"grid",gridTemplateColumns:"110px 1fr",gap:8}}><span style={{color:"var(--muted)"}}>{new Date(entry.ts).toLocaleString("tr-TR")}</span><span><b>{entry.userName||"Sistem"}</b> · {entry.label}: {entry.from} → {entry.to}</span></div>)}</div>
  ...
  return <Modal title={`${ticketNumber(ticket)} · ${ticket.title}`} onClose={onClose} wide>
    {editing?<div>
      ...
      {(ticket.customer||ticket.module||ticket.subModule||ticket.functionButton||ticket.pageUrl||(ticket.requestRequirements||[]).length||(ticket.completionCriteria||[]).length)&&<div className="ticket-detail-card" style={{border:"1px solid var(--border)",borderRadius:12,padding:13,background:"#fff",marginBottom:16}}>
      ...
      {mailStatus.message&&<div className={`ui-feedback-message ${mailStatus.error?"is-danger":"is-success"}`} style={{fontSize:11,fontWeight:700,color:mailStatus.error?"var(--danger)":"var(--success)",background:mailStatus.error?"color-mix(in srgb, var(--danger) 9%, white 91%)":"color-mix(in srgb, var(--success) 10%, white 90%)",borderRadius:8,padding:"8px 10px",marginBottom:12}}>{mailStatus.message}</div>}
      <div className="ticket-detail-card" style={{border:"1.5px solid var(--border)",borderRadius:12,padding:14,background:"var(--surface-soft)",marginBottom:16}}>
        ...
        {jiraKey&&<div>
          ...
          {(jira?.description||ticket.jiraDescription)&&<div className="ui-muted-note" style={{fontSize:11,color:"var(--muted)",lineHeight:1.55,whiteSpace:"pre-wrap",background:"#fff",borderRadius:8,padding:"9px 10px",marginBottom:9}}>{jira?.description||ticket.jiraDescription}</div>}
