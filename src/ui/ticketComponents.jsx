...
export function TicketsPage({
  ...
  return <div style={{padding:"clamp(16px,4vw,28px)",flex:1,overflow:"auto"}}>
    ...
    {ticketView==="list"?<TicketListTable rows={sortedFiltered} people={state.people} customerMode={customerMode} showProject={!customerMode} onOpen={({ticket,projectId})=>setModal({type:"detail",projectId,data:ticket})} onStatusChange={(projectId,id,status)=>update(projectId,id,{status})} onDelete={(projectId,id)=>{if(confirm("Ticket silinsin mi?"))remove(projectId,id);}} canDelete={(ticket)=>isAdmin||ticket.author===currentUser.name}/>:<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(360px,100%),1fr))",gap:10}}>{sortedFiltered.map(({ticket,project,projectId})=>{const age=Math.max(0,daysDiff(ticket.ts));const assignee=state.people.find(p=>p.id===ticket.assignedTo);return <div className="compact-list-card" key={`${projectId}-${ticket.id}`} onClick={()=>setModal({type:"detail",projectId,data:ticket})} style={{borderTop:`3px solid ${project?.color||"var(--accent, #0b8a94)"}`,padding:"14px 15px",cursor:"pointer"}}>
