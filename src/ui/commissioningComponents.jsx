...
export function CommissioningPanel({ project, canEdit, onChange }) {
  ...
  return <div style={{flex:1,overflow:"auto",padding:"clamp(14px,3vw,24px)",background:"var(--surface-soft)"}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap",marginBottom:14}}>
      <div><h3 style={{margin:0,fontSize:17}}>Toplu Devreye Alma Takibi</h3><div style={{fontSize:11,color:"var(--muted)",marginTop:3}}>T\u00fcm hiyerar\u015fi tek listede; filtreleyin, i\u015faretleyin veya Excel ile y\u00f6netin.</div></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:9,marginBottom:13}}>
      {[["\u0130lerleme",`${percent}%`,"var(--accent)"],["Devrede",`${commissioned}/${machines.length}`,"var(--success)"],["Fiziksel",physical,"var(--accent)"],["Sanal",virtual,"var(--accent)"],["Bekleyen",machines.length-commissioned,"var(--warning)"]].map(([label,value,color])=><div key={label} style={{background:"#fff",border:"1px solid var(--border)",borderRadius:11,padding:12}}><div style={{fontSize:10,color:"var(--muted)"}}>{label}</div><div style={{fontSize:21,fontWeight:800,color,marginTop:2}}>{value}</div></div>)}
    </div>
    <div style={{height:10,background:"var(--border)",borderRadius:10,overflow:"hidden",marginBottom:13}}><div style={{height:"100%",width:`${percent}%`,background:"linear-gradient(90deg,var(--accent),var(--success))",transition:"width .25s"}}/></div>
    <div style={{display:"flex",gap:6,marginBottom:13}}>{[["all","T\u00fcm\u00fc"],["pending","Devreye Al\u0131nacak"],["done","Devreye Al\u0131nan"]].map(([id,label])=><button key={id} onClick={()=>setFilter(id)} style={{border:0,borderRadius:8,padding:"7px 11px",background:filter===id?"var(--accent)":"#fff",color:filter===id?"#fff":"var(--muted)",fontSize:11,fontWeight:700,cursor:"pointer"}}>{label}</button>)}</div>
    {showForm&&<div style={{background:"#fff",border:"1px solid var(--border)",borderRadius:13,padding:15,marginBottom:14}}><div style={{fontSize:12,fontWeight:850,marginBottom:10}}>{editingMachineId?"Makine Kayd\u0131n\u0131 D\u00fczenle":"Yeni Makine Kayd\u0131"}</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:9}}>{[["sector","Sekt\u00f6r"],["productionCenter","\u00dcretim Merkezi"],["workplace","\u0130\u015fyeri"],["line","Hat"],["name","Makine Ad\u0131"],["code","Makine Kodu"]].map(([key,label])=><Field key={key} label={label}><input style={iStyle} value={form[key]} onChange={event=>setForm(current=>({...current,[key]:event.target.value}))}/></Field>)}<Field label="Tip"><select style={iStyle} value={form.type} onChange={event=>setForm(current=>({...current,type:event.target.value}))}><option value="physical">Fiziksel</option><option value="virtual">Sanal</option></select></Field></div><Field label="A\u00e7\u0131klama"><input style={iStyle} value={form.note} onChange={event=>setForm(current=>({...current,note:event.target.value}))}/></Field><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}><label style={{fontSize:12,fontWeight:700}}><input type="checkbox" checked={form.commissioned} onChange={event=>setForm(current=>({...current,commissioned:event.target.checked}))}/> Devreye al\u0131nd\u0131</label><Btn onClick={addMachine}>{editingMachineId?"Değişiklikleri Kaydet":"Kaydet"}</Btn></div></div>}
    <div style={{overflowX:"auto",background:"#fff",border:"1px solid var(--border)",borderRadius:13}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:900}}><thead><tr>{["Durum","Sekt\u00f6r","\u00dcretim Merkezi","\u0130\u015fyeri","Hat","Makine","Tip","A\u00e7\u0131klama",""].map(label=><th key={label} style={{padding:"10px 9px",fontSize:10,color:"var(--muted)",background:"var(--surface-soft)",textAlign:"left"}}>{label}</th>)}</tr></thead><tbody>{visibleRows.map(row=><tr key={row.machine.id} style={{borderTop:"1px solid #EEF2F7"}}><td style={{padding:9}}><input type="checkbox" checked={Boolean(row.machine.commissioned)} disabled={!canEdit} onChange={event=>update(row.machine.id,current=>({...current,commissioned:event.target.checked,commissionedAt:event.target.checked?todayStr():""}))}/></td>{[row.sector,row.productionCenter,row.workplace,row.line].map((value,index)=><td key={index} style={{padding:9,fontSize:11,color:"var(--muted)"}}>{value}</td>)}<td style={{padding:9}}><div style={{fontSize:11,fontWeight:800}}>{row.machine.name}</div><div style={{fontSize:9,color:"var(--muted)"}}>{row.machine.code||"Kod yok"}</div></td><td style={{padding:9,fontSize:11}}>{row.machine.type==="virtual"?"Sanal":"Fiziksel"}</td><td style={{padding:9,minWidth:190}}>{canEdit?<input style={{...iStyle,padding:"6px 8px",fontSize:10}} value={row.machine.note||""} onChange={event=>update(row.machine.id,current=>({...current,note:event.target.value}))}/>:<span style={{fontSize:10,color:"var(--muted)"}}>{row.machine.note||"-"}</span>}</td><td style={{padding:9}}>{canEdit&&<div style={{display:"flex",gap:7}}><button onClick={()=>editMachine(row)} style={{border:0,background:"transparent",color:"var(--accent)",cursor:"pointer",fontSize:11}}>D\u00fczenle</button><button onClick={()=>confirm("Makine silinsin mi?")&&remove(row.machine.id)} style={{border:0,background:"transparent",color:"var(--danger)",cursor:"pointer",fontSize:11}}>Sil</button></div>}</td></tr>)}</tbody></table></div>
    {!visibleRows.length&&<div style={{padding:35,textAlign:"center",color:"var(--muted)"}}>Bu filtrede kay\u0131t bulunmuyor.</div>}
  </div>;
}
