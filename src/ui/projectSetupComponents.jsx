...
function ProjectEffortPanel({project,state,people}) {
  ...
  return <div style={{flex:1,overflow:"auto",padding:"18px 22px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:14}}><div><h3 style={{margin:0,fontSize:16}}>Proje Efor Merkezi</h3><p style={{margin:"3px 0 0",fontSize:11,color:"var(--muted)"}}>Görev, aksiyon ve saha ziyaretlerinden gelen tüm eforlar.</p></div><Btn small onClick={exportXlsx}>XLSX İndir</Btn></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:9,marginBottom:14}}>
      {[["Toplam Efor",`${total} sa`,"#7C3AED"],["Planlanan",`${planned} sa`,"#0369A1"],["Fark",`${Math.round((total-planned)*10)/10} sa`,total>planned?"var(--danger)":"var(--success)"],["Kayıt",rows.length,"var(--accent)"]].map(([label,value,color])=><div key={label} style={{background:"#fff",border:"1px solid var(--border)",borderTop:`3px solid ${color}`,borderRadius:12,padding:13}}><div style={{fontSize:10,color:"var(--muted)"}}>{label}</div><b style={{display:"block",fontSize:22,color,marginTop:3}}>{value}</b></div>)}
    </div>
    {byPerson.length>0&&<div style={{background:"#fff",border:"1px solid var(--border)",borderRadius:12,padding:13,marginBottom:12}}><div style={{fontSize:11,fontWeight:800,marginBottom:9}}>Kişi Bazlı Dağılım</div><div style={{display:"flex",gap:7,flexWrap:"wrap"}}>{byPerson.map(([name,hours])=><span key={name} style={{background:"var(--surface-soft)",color:"var(--accent)",borderRadius:8,padding:"6px 9px",fontSize:10,fontWeight:700}}>{name}: {hours} sa</span>)}</div></div>}
    <div style={{display:"grid",gap:7}}>{rows.map(row=><div key={row.id} className="project-effort-row" style={{background:"#fff",border:"1px solid var(--border)",borderRadius:10,padding:"10px 12px",display:"grid",gridTemplateColumns:"110px minmax(160px,1fr) 150px 90px 75px",gap:9,alignItems:"center"}}><span style={{fontSize:9,fontWeight:800,color:"var(--accent)",background:"var(--accent-ink)",borderRadius:6,padding:"4px 6px",textAlign:"center"}}>{row.source}</span><div style={{minWidth:0}}><b style={{fontSize:11}}>{row.item}</b>{row.note&&<div style={{fontSize:9,color:"var(--muted)",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{row.note}</div>}</div><span style={{fontSize:10,color:"var(--muted)"}}>{row.person}</span><span style={{fontSize:10,color:"var(--muted)"}}>{fmt(row.date)}</span><b style={{fontSize:12,color:"#7C3AED",textAlign:"right"}}>{row.hours} sa</b></div>)}{!rows.length&&<div style={{padding:35,textAlign:"center",color:"var(--muted)",background:"#fff",border:"1px dashed var(--muted)",borderRadius:12}}>Bu projede henüz efor kaydı yok.</div>}</div>
  </div>;
}
...
function LessonsLearnedPanel({project,onChange,canEdit,currentUser}) {
  ...
  return <div>
    {canEdit&&<div style={{background:"#fff",border:"1px solid var(--border)",borderRadius:14,padding:15,marginBottom:13}}>
    ...
    <div style={{display:"grid",gap:9}}>{lessons.map(item=><div key={item.id} style={{background:"#fff",border:"1px solid var(--border)",borderLeft:"4px solid #7C3AED",borderRadius:12,padding:14}}><div style={{display:"flex",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}><b>{item.title}</b><span style={{fontSize:9,fontWeight:900,color:"var(--accent)",background:"var(--surface-soft)",borderRadius:7,padding:"4px 7px"}}>{item.category}</span></div><div style={{fontSize:12,color:"var(--text)",lineHeight:1.55,whiteSpace:"pre-wrap",marginTop:8}}>{item.lesson}</div>{item.prevention&&<div style={{fontSize:11,color:"var(--success)",background:"color-mix(in srgb, var(--success) 10%, white 90%)",borderRadius:9,padding:"8px 10px",marginTop:9}}><b>Önleyici aksiyon:</b> {item.prevention}</div>}<div style={{fontSize:10,color:"var(--muted)",marginTop:8}}>{item.authorName} · {new Date(item.createdAt||now()).toLocaleString("tr-TR")}</div>{canEdit&&<button onClick={()=>onChange({lessonsLearned:lessons.filter(lesson=>lesson.id!==item.id)})} style={{marginTop:8,border:0,background:"transparent",color:"var(--danger)",fontSize:10,fontWeight:800,cursor:"pointer"}}>Sil</button>}</div>)}</div>
    {!lessons.length&&<div style={{padding:36,textAlign:"center",border:"1px dashed var(--muted)",borderRadius:12,color:"var(--muted)"}}>Bu proje için henüz öğrenilmiş ders yok.</div>}
  </div>;
}
