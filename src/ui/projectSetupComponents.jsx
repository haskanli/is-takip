...
const TRAINING_SCOPES = ["OperatÃ¶r EÄŸitimi","YÃ¶netici EÄŸitimi","HatÄ±rlatma EÄŸitimi","Proje Devri EÄŸitimi","SÃ¼per KullanÄ±cÄ± EÄŸitimi","Teknik EÄŸitim","DiÄŸer"];
const COST_CATEGORIES = ["Saha YakÄ±t","Yetkili Servis Ä°ÅŸÃ§ilik","DonanÄ±m","Lisans","Konaklama","UlaÅŸÄ±m","DiÄŸer"];
const DEFAULT_ACTIVE_MODULES = ["Ãœretim Takip","BakÄ±m","Kalite","Ä°zlenebilirlik","SatÄ±n Alma","Connected Supplier"];
...
const TICKET_STATUSES = ["AÃ§Ä±k","Operasyon Ä°ncelemesinde","ÃœrÃ¼n DeÄŸerlendirmesinde","Jira'da Ã‡alÄ±ÅŸÄ±lÄ±yor","Operasyon Testinde","Test BaÅŸarÄ±sÄ±z","YayÄ±na HazÄ±r","MÃ¼ÅŸteri OnayÄ±nda","MÃ¼ÅŸteri Reddetti","TamamlandÄ±","Beklemede","Ä°ptal Edildi"];
const TICKET_CATEGORIES = ["Operasyonel","Bug","GeliÅŸtirme","Entegrasyon","Ä°yileÅŸtirme","Veri","EÄŸitim","DiÄŸer"];
const ORG_LEVELS = [
  { id:"ceo", label:"CEO", rank:1 },
  { id:"coo", label:"COO", rank:2 },
  { id:"operations_director", label:"Operasyon DirektÃ¶rÃ¼", rank:3 },
  { id:"project_director", label:"Proje MÃ¼dÃ¼rÃ¼", rank:4 },
  { id:"project_manager", label:"Proje YÃ¶neticisi", rank:5 },
  { id:"process_lead", label:"SÃ¼reÃ§ Lideri", rank:6 },
  { id:"field_engineer", label:"Saha MÃ¼hendisi", rank:7 },
];


function RemoteAccessPanel({project,setState,isAdmin,canManage,authHeaders,updateApiStateVersion}) {
  const empty={name:"",url:"",username:"",password:"",status:"HazÄ±r",routing:"",note:""};
  ...
  useEffect(()=>{
    ...
    authHeaders().then(headers=>fetch(apiUrl(endpoint),{headers})).then(async response=>{
      ...
      if(!response.ok)throw new Error(result.error||"EriÅŸim kasasÄ± yÃ¼klenemedi.");
  ...
  const submit=async()=>{
    ...
    try{
      ...
      const result=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(result.error||"EriÅŸim kaydÄ± kaydedilemedi.");
  ...
  const remove=async id=>{
    if(!confirm("EriÅŸim kaydÄ± silinsin mi?"))return;
    setError("");
    try{
      const response=await fetch(apiUrl(`${endpoint}/${encodeURIComponent(id)}`),{method:"DELETE",headers:await authHeaders()});
      const result=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(result.error||"EriÅŸim kaydÄ± silinemedi.");
      updateApiStateVersion?.(Number(result.version||0));
  ...
  return <div style={{flex:1,overflow:"auto",padding:"18px 22px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:14}}>
      <div><h3 style={{margin:0,fontSize:16}}>Uzaktan EriÅŸim KasasÄ±</h3><p style={{margin:"3px 0 0",fontSize:11,color:"#64748B"}}>Parolalar ÅŸifreli saklanÄ±r; projeye dahil kullanÄ±cÄ±lar gÃ¶rÃ¼ntÃ¼leyebilir, yalnÄ±zca proje yÃ¶neticileri ve adminler dÃ¼zenleyebilir.</p></div>
      {canEdit&&<Btn small onClick={openAdd}>+ EriÅŸim Ekle</Btn>}
    </div>
    ...
    {loading&&<div style={{padding:28,textAlign:"center",color:"#64748B"}}>Åifre kasasÄ± yÃ¼kleniyor...</div>}
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(320px,100%),1fr))",gap:10,marginBottom:14}}>
      {!loading&&records.map(item=><div key={item.id} style={{background:"#fff",border:"1px solid #E2E8F0",borderTop:`3px solid ${item.status==="HazÄ±r"?"#059669":"#EA6C00"}`,borderRadius:13,padding:14}}>
        <div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"center"}}><b style={{fontSize:13}}>{item.name}</b><span style={{fontSize:9,fontWeight:850,color:item.status==="HazÄ±r"?"#047857":"#C2410C",background:item.status==="HazÄ±r"?"#ECFDF5":"#FFF7ED",padding:"3px 6px",borderRadius:7}}>{item.status}</span></div>
        <div style={{display:"grid",gap:7,marginTop:10}}>
          {[["Adres",item.url,"url"],["KullanÄ±cÄ±",item.username,"username"]].filter(([,value])=>value).map(([label,value,key])=><div key={key} style={{display:"flex",alignItems:"center",gap:7,background:"#F8FAFC",borderRadius:8,padding:"7px 9px"}}><span style={{fontSize:9,color:"#64748B",width:54}}>{label}</span><b style={{fontSize:10,flex:1,wordBreak:"break-all"}}>{value}</b><button onClick={()=>copy(value,`${item.id}-${key}`)} style={{border:0,background:"#EEF2FF",color:"#4338CA",borderRadius:6,padding:"4px 7px",fontSize:9,fontWeight:800,cursor:"pointer"}}>{copied===`${item.id}-${key}`?"KopyalandÄ±":"Kopyala"}</button></div>)}
          {item.password&&<div style={{display:"flex",alignItems:"center",gap:7,background:"#FFF7ED",borderRadius:8,padding:"7px 9px"}}><span style={{fontSize:9,color:"#9A3412",width:54}}>Parola</span><b style={{fontSize:11,flex:1,letterSpacing:visibleSecrets[item.id]?0:2}}>{visibleSecrets[item.id]?item.password:"â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢"}</b><button onClick={()=>setVisibleSecrets(current=>({...current,[item.id]:!current[item.id]}))} style={{border:0,background:"transparent",color:"#C2410C",fontSize:9,fontWeight:800,cursor:"pointer"}}>{visibleSecrets[item.id]?"Gizle":"GÃ¶ster"}</button><button onClick={()=>copy(item.password,`${item.id}-password`)} style={{border:0,background:"#FFEDD5",color:"#9A3412",borderRadius:6,padding:"4px 7px",fontSize:9,fontWeight:800,cursor:"pointer"}}>{copied===`${item.id}-password`?"KopyalandÄ±":"Kopyala"}</button></div>}
        </div>
        {item.routing&&<div style={{fontSize:10,color:"#475569",lineHeight:1.5,marginTop:9}}><b>VPN / YÃ¶nlendirme:</b> {item.routing}</div>}
        {item.note&&<div style={{fontSize:10,color:"#64748B",lineHeight:1.5,marginTop:5}}>{item.note}</div>}
        {canEdit&&<div style={{display:"flex",gap:9,marginTop:10}}><button onClick={()=>openEdit(item)} style={{border:0,background:"transparent",color:"#4A6CF7",fontSize:10,fontWeight:800,cursor:"pointer"}}>DÃ¼zenle</button><button onClick={()=>remove(item.id)} style={{border:0,background:"transparent",color:"#E11D48",fontSize:10,fontWeight:800,cursor:"pointer"}}>Sil</button></div>}
      </div>)}
    </div>
    {!loading&&!records.length&&!showForm&&<div style={{padding:38,textAlign:"center",border:"1px dashed #CBD5E1",borderRadius:12,color:"#94A3B8"}}>Uzaktan eriÅŸim kaydÄ± bulunmuyor.</div>}
    {showForm&&canEdit&&<div style={{background:"#fff",border:"1.5px solid #CBD5E1",borderRadius:14,padding:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><b style={{fontSize:13}}>{editingId?"EriÅŸim KaydÄ±nÄ± DÃ¼zenle":"Yeni EriÅŸim KaydÄ±"}</b><button onClick={()=>setShowForm(false)} style={{border:0,background:"transparent",fontSize:18,color:"#94A3B8",cursor:"pointer"}}>Ã—</button></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:9}}>
        {[["name","Sistem / Sunucu","text"],["url","BaÄŸlantÄ± Adresi","text"],["username","KullanÄ±cÄ± AdÄ±","text"],["password","Parola","password"],["routing","VPN / YÃ¶nlendirme","text"]].map(([key,label,type])=><Field key={key} label={label}><input type={type} autoComplete="off" style={iStyle} value={form[key]} onChange={event=>setForm(current=>({...current,[key]:event.target.value}))}/></Field>)}
        <Field label="Durum"><select style={iStyle} value={form.status} onChange={event=>setForm(current=>({...current,status:event.target.value}))}>{["HazÄ±r","Test Bekliyor","EriÅŸim Yok","SÃ¼resi Doldu"].map(status=><option key={status}>{status}</option>)}</select></Field>
      </div>
      <Field label="Not"><input style={iStyle} value={form.note} onChange={event=>setForm(current=>({...current,note:event.target.value}))}/></Field>
      <div style={{display:"flex",justifyContent:"flex-end",gap:7}}><Btn variant="ghost" disabled={saving} onClick={()=>setShowForm(false)}>Ä°ptal</Btn><Btn disabled={saving} onClick={submit}>{saving?"Kaydediliyor...":editingId?"GÃ¼ncelle":"Kaydet"}</Btn></div>
    </div>}
  </div>;
}
...
function ProjectEffortPanel({project,state,people}) {
  const taskRows=project.milestones.flatMap(milestone=>milestone.tasks.flatMap(task=>(task.timeEntries||[]).map(entry=>({
    id:`task-${task.id}-${entry.id}`,source:"GÃ¶rev",item:task.title,person:people.find(person=>person.id===(entry.userId||task.assignee))?.name||entry.user||"Bilinmiyor",date:entry.date||entry.ts,hours:Number(entry.hours)||0,note:entry.note||"",planned:Number(task.estimatedHours)||0,
  }))));
  ...
  const visitRows=(state.fieldPlans||[]).filter(plan=>plan.projectId===project.id&&(plan.status==="completed"||plan.completedAt)).map(plan=>({
    id:`visit-${plan.id}`,source:plan.workType==="remote"?"Uzaktan Ã‡alÄ±ÅŸma":"Saha Ziyareti",item:plan.customer||project.name,person:people.find(person=>person.id===plan.userId)?.name||"Bilinmiyor",date:plan.date||plan.completedAt,hours:fieldPlanHours(plan),note:plan.visitNotes||"",planned:0,
  }));
  ...
  const exportXlsx=()=>{
    const data=[["Kaynak","Ä°ÅŸ / KayÄ±t","KiÅŸi","Tarih","Efor (Saat)","Planlanan (Saat)","AÃ§Ä±klama"],...rows.map(row=>[row.source,row.item,row.person,row.date||"",row.hours,row.planned,row.note])];
...
const reportScheduleStatus = (item) => {
  ...
  if(item.lastSentAt)return `Son gÃ¶nderim: ${new Date(item.lastSentAt).toLocaleString("tr-TR",{timeZone:"Europe/Istanbul"})}`;
  return `Sonraki gÃ¶nderim: ${item.nextRunAt?new Date(item.nextRunAt).toLocaleString("tr-TR",{timeZone:"Europe/Istanbul"}):"-"}`;
};
  ...
  return <details style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:12,padding:13}}>
    <summary style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",cursor:"pointer",listStyle:"none"}}>
      <div style={{flex:1,minWidth:180}}>
        ...
        <div style={{fontSize:10,color:"#64748B",marginTop:3}}>{item.reportType==="jira_newsletter"?"Jira Done geliÅŸtirme bÃ¼lteni":"Proje durum raporu"} Â· {item.frequency==="weekly"?"HaftalÄ±k":"AylÄ±k"} Â· {item.time} (Ä°stanbul) Â· {(item.recipients||[]).join(", ")}</div>
...
function ProjectOverviewPanel({project,onChange,canEdit}) {
  ...
  const contacts=(project.raciContacts||[]).filter(item=>item.side==="MÃ¼ÅŸteri");
  const totalTasks=project.milestones.flatMap(milestone=>milestone.tasks||[]).length;
  const doneTasks=project.milestones.flatMap(milestone=>milestone.tasks||[]).filter(task=>task.status==="TamamlandÄ±").length;
  ...
  const openRisks=(project.risks||[]).filter(risk=>!["KapalÄ±","Kapal\u0131"].includes(risk.status)).length;
  ...
  const statCards=[["Tamamlama",`%${progress}`,"#4F46E5",`${doneTasks}/${totalTasks} gÃ¶rev`],["Proje SaÄŸlÄ±ÄŸÄ±",`%${readinessScore(project)}`,readinessScore(project)>=Number(project.readinessThreshold||80)?"#059669":"#E11D48","hazÄ±rlÄ±k skoru"],["Geciken",lateTasks,"#EA6C00","aÃ§Ä±k termin"],["Risk",openRisks,"#E11D48","aÃ§Ä±k risk"]];
  return <div style={{display:"grid",gap:14}}>
    ...
    <div className="admin-main-grid" style={{display:"none",gridTemplateColumns:"1.15fr .85fr",gap:14}}>
      <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:18,padding:18}}>
        <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",marginBottom:13}}><div><h3 style={{margin:0,fontSize:15}}>MÃ¼ÅŸteri Bilgileri</h3><p style={{margin:"4px 0 0",fontSize:11,color:"#64748B"}}>Logo, web sitesi, konum ve aktif modÃ¼ller.</p></div><div style={{display:"flex",gap:7}}>{customer.website&&<a href={customer.website.startsWith("http")?customer.website:`https://${customer.website}`} target="_blank" rel="noreferrer" style={{fontSize:10,fontWeight:850,color:"#2563EB",background:"#DBEAFE",borderRadius:9,padding:"7px 9px",textDecoration:"none"}}>Web Sitesi</a>}{canEdit&&<button onClick={()=>setEditing(value=>!value)} style={{border:0,borderRadius:9,padding:"7px 9px",background:editing?"#111827":"#EEF2FF",color:editing?"#fff":"#4338CA",fontSize:10,fontWeight:900,cursor:"pointer"}}>{editing?"Kapat":"DÃ¼zenle"}</button>}</div></div>
        {editing&&canEdit&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:9,marginBottom:14}}><Field label="MÃ¼ÅŸteri AdÄ±"><input style={iStyle} value={customer.name||""} onChange={event=>updateCustomer("name",event.target.value)} placeholder="Firma adÄ±"/></Field><Field label="MÃ¼ÅŸteri Web Sitesi"><input style={iStyle} value={customer.website||""} onChange={event=>updateCustomer("website",event.target.value)} placeholder="https://firma.com"/></Field><Field label="MÃ¼ÅŸteri Logo"><div style={{display:"grid",gap:6}}><input style={iStyle} value={(customer.logoUrl||"").startsWith("data:")?"":customer.logoUrl||""} onChange={event=>updateCustomer("logoUrl",event.target.value)} placeholder="https://.../logo.png"/><button type="button" onClick={()=>fileInput.current?.click()} style={{border:0,background:"#EEF2FF",color:"#4338CA",borderRadius:9,padding:"7px 9px",fontSize:10,fontWeight:900,cursor:"pointer",textAlign:"left"}}>veya dosya yÃ¼kle</button><input ref={fileInput} type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" hidden onChange={event=>uploadLogo(event.target.files?.[0])}/></div></Field><Field label="Ä°l"><input style={iStyle} value={location.city||""} onChange={event=>updateLocation("city",event.target.value)}/></Field><Field label="Ä°lÃ§e"><input style={iStyle} value={location.district||""} onChange={event=>updateLocation("district",event.target.value)}/></Field><Field label="Adres"><input style={iStyle} value={location.address||""} onChange={event=>updateLocation("address",event.target.value)} placeholder="AÃ§Ä±k adres"/></Field></div>}
...
function ReadinessPanel({project,checklist,score,threshold,canEdit,onChange,updateItem}) {
  ...
  const statusMeta={
    ready:["HazÄ±r","#059669","#ECFDF5"],
    partial:["KÄ±smi","#EA6C00","#FFF7ED"],
    not_ready:["HazÄ±r DeÄŸil","#E11D48","#FFF1F2"],
  ...
  return <div style={{display:"grid",gap:14}}>
    <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:20,padding:18,boxShadow:"0 10px 28px rgba(15,23,42,.04)"}}>
      <div style={{display:"flex",gap:18,alignItems:"center",flexWrap:"wrap"}}>
        ...
        <div style={{flex:1,minWidth:230}}><div style={{fontSize:11,fontWeight:900,color:"#64748B",letterSpacing:1,textTransform:"uppercase"}}>Proje SaÄŸlÄ±ÄŸÄ±</div><h3 style={{margin:"5px 0 6px",fontSize:22}}>{score>=threshold?"Proje BaÅŸlamaya ElveriÅŸli":"BaÅŸlamadan Ã–nce TamamlanmalÄ±"}</h3><p style={{margin:0,fontSize:12,color:"#64748B",lineHeight:1.65}}>MES projeleri iÃ§in kapsam, veri, entegrasyon, OT altyapÄ±, saha hazÄ±rlÄ±ÄŸÄ± ve kabul maddeleri tek yerde takip edilir. EÅŸik deÄŸer <b>{threshold}</b>; 80 altÄ± projelerde saha uygulamasÄ±na geÃ§meden Ã¶nce eksikler kapatÄ±lmalÄ±.</p></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,94px)",gap:8}}>{[["ready","HazÄ±r"],["partial","KÄ±smi"],["not_ready","Eksik"],["unanswered","Bekleyen"]].map(([key,label])=>{const [,color,bg]=statusMeta[key];return <div key={key} style={{background:bg,borderRadius:13,padding:10,textAlign:"center"}}><b style={{display:"block",fontSize:20,color}}>{counts[key]||0}</b><span style={{fontSize:9,color,fontWeight:850}}>{label}</span></div>;})}</div>
      </div>
    </div>
    <div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:2}}>{categories.map(item=><button key={item} onClick={()=>setCategory(item)} style={{border:0,borderRadius:999,padding:"8px 12px",background:category===item?"#111827":"#F1F5F9",color:category===item?"#fff":"#64748B",fontSize:10,fontWeight:900,cursor:"pointer",whiteSpace:"nowrap"}}>{item==="all"?"TÃ¼m Kategoriler":item}</button>)}</div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(360px,100%),1fr))",gap:10}}>{filtered.map(item=>{const [label,color,bg]=statusMeta[item.status]||statusMeta.unanswered;return <div key={item.id} style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:16,padding:14,boxShadow:"0 6px 18px rgba(15,23,42,.035)"}}>
      <div style={{display:"flex",gap:10,alignItems:"flex-start"}}><span style={{width:34,height:34,borderRadius:12,background:bg,color,display:"grid",placeItems:"center",flexShrink:0}}><Icon name={item.status==="ready"?"check":"clock"} size={16}/></span><div style={{flex:1,minWidth:0}}><div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap"}}><span style={{fontSize:9,fontWeight:900,color:"#4338CA",background:"#EEF2FF",borderRadius:999,padding:"3px 7px"}}>{item.category}</span><span style={{fontSize:9,fontWeight:900,color,background:bg,borderRadius:999,padding:"3px 7px"}}>{label}</span><span style={{fontSize:9,color:"#94A3B8"}}>AÄŸÄ±rlÄ±k {item.weight}</span></div>{canEdit?<input style={{...iStyle,border:0,padding:"8px 0",fontWeight:850,fontSize:12,background:"transparent"}} value={item.text} onChange={event=>updateItem(item.id,{text:event.target.value})}/>:<b style={{display:"block",fontSize:12,marginTop:8,lineHeight:1.45}}>{item.text}</b>}</div></div>
      {canEdit&&<div style={{display:"grid",gridTemplateColumns:"1fr 72px auto",gap:8,alignItems:"center",marginTop:10}}><select style={iStyle} value={item.status} onChange={event=>updateItem(item.id,{status:event.target.value})}><option value="unanswered">Bekliyor</option><option value="ready">HazÄ±r</option><option value="partial">KÄ±smi</option><option value="not_ready">HazÄ±r DeÄŸil</option></select><input type="number" min="1" max="100" style={iStyle} value={item.weight} onChange={event=>updateItem(item.id,{weight:event.target.value})}/><button title="Sil" onClick={()=>onChange({readinessChecklist:checklist.filter(entry=>entry.id!==item.id)})} style={{border:0,background:"#FFF1F2",color:"#E11D48",borderRadius:10,padding:"9px 11px",cursor:"pointer"}}>Sil</button></div>}
      <textarea disabled={!canEdit} style={{...iStyle,minHeight:64,resize:"vertical",marginTop:9,fontSize:11,background:canEdit?"#F8FAFC":"#fff"}} value={item.note||""} onChange={event=>updateItem(item.id,{note:event.target.value})} placeholder="KanÄ±t, eksik veya aksiyon notu"/>
    </div>;})}</div>
    {canEdit&&<div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Btn small onClick={()=>onChange({readinessChecklist:[...checklist,{id:uid(),category:"Ã–zel",text:"Yeni kontrol maddesi",weight:1,status:"unanswered",note:""}]})}>+ Kontrol Maddesi</Btn><Btn small variant="secondary" onClick={reset}>MES Åablonunu SÄ±fÄ±rla</Btn></div>}
  </div>;
}
...
function TrainingPanel({project,onChange,canEdit,people}) {
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",marginBottom:12,flexWrap:"wrap"}}><div><h3 style={{margin:0,fontSize:15}}>EÄŸitimler</h3><p style={{margin:"4px 0 0",fontSize:11,color:"#64748B"}}>Proje kapsamÄ±ndaki eÄŸitimleri, katÄ±lÄ±mcÄ±larÄ± ve devir notlarÄ±nÄ± takip edin.</p></div><input style={{...iStyle,width:240}} value={search} onChange={event=>setSearch(event.target.value)} placeholder="EÄŸitimlerde ara..."/></div>
    {canEdit&&<div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:14,padding:15,marginBottom:13,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:9}}><Field label="Kapsam"><select style={iStyle} value={form.scope} onChange={event=>update("scope",event.target.value)}>{TRAINING_SCOPES.map(scope=><option key={scope}>{scope}</option>)}</select></Field><Field label="BaÅŸlÄ±k"><input style={iStyle} value={form.title} onChange={event=>update("title",event.target.value)} placeholder="Ã–rn. OperatÃ¶r vardiya eÄŸitimi"/></Field><Field label="Tarih"><input type="date" style={iStyle} value={form.date} onChange={event=>update("date",event.target.value)}/></Field><Field label="EÄŸitmen"><select style={iStyle} value={form.trainerId} onChange={event=>update("trainerId",event.target.value)}><option value="">- SeÃ§ -</option>{people.map(person=><option key={person.id} value={person.id}>{person.name}</option>)}</select></Field><Field label="KatÄ±lÄ±mcÄ±lar"><input style={iStyle} value={form.participants} onChange={event=>update("participants",event.target.value)} placeholder="VirgÃ¼l ile yazÄ±n"/></Field><Field label="Not"><input style={iStyle} value={form.notes} onChange={event=>update("notes",event.target.value)} placeholder="Kapsam, eksik kalanlar, sonraki eÄŸitim"/></Field><div style={{alignSelf:"end"}}><Btn onClick={save}>EÄŸitimi Kaydet</Btn></div></div>}
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(300px,100%),1fr))",gap:10}}>{filtered.map(item=>{const trainer=people.find(person=>person.id===item.trainerId);return <div key={item.id} style={{background:"#fff",border:"1px solid #E2E8F0",borderTop:"3px solid #7C3AED",borderRadius:13,padding:14}}><div style={{display:"flex",justifyContent:"space-between",gap:8}}><b style={{fontSize:13}}>{item.title}</b><span style={{fontSize:9,fontWeight:850,color:"#6D28D9",background:"#F5F3FF",borderRadius:7,padding:"3px 6px"}}>{item.scope}</span></div><div style={{fontSize:11,color:"#64748B",lineHeight:1.6,marginTop:8}}>{fmt(item.date)}{trainer?` Â· ${trainer.name}`:""}{item.participants?.length?` Â· ${item.participants.length} katÄ±lÄ±mcÄ±`:""}{item.notes?` Â· ${item.notes}`:""}</div>{canEdit&&<button onClick={()=>onChange({trainings:trainings.filter(entry=>entry.id!==item.id)})} style={{marginTop:9,border:0,background:"transparent",color:"#E11D48",fontSize:10,fontWeight:800,cursor:"pointer"}}>Sil</button>}</div>;})}</div>
    {!filtered.length&&<div style={{padding:36,textAlign:"center",border:"1px dashed #CBD5E1",borderRadius:12,color:"#94A3B8"}}>EÄŸitim kaydÄ± bulunmuyor.</div>}
  </div>;
}
...
function ProjectCostPanel({project,onChange,state,canEdit}) {
  ...
  const empty={category:"Yetkili Servis Ä°ÅŸÃ§ilik",description:"",date:todayStr(),amountUsd:"",amountTry:"",note:""};
  const [form,setForm]=useState(empty);
...
function LessonsLearnedPanel({project,onChange,canEdit,currentUser}) {
  ...
  const save=()=>{
    ...
    onChange({lessonsLearned:[{id:uid(),...form,title:form.title.trim()||"Ã–ÄŸrenilmiÅŸ ders",lesson:form.lesson.trim(),prevention:form.prevention.trim(),authorId:currentUser?.id||"",authorName:currentUser?.name||"KullanÄ±cÄ±",createdAt:now()},...lessons]});
  ...
  return <div>
    {canEdit&&<div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:14,padding:15,marginBottom:13}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:9}}><Field label="BaÅŸlÄ±k"><input style={iStyle} value={form.title} onChange={event=>setForm({...form,title:event.target.value})}/></Field><Field label="Kategori"><select style={iStyle} value={form.category} onChange={event=>setForm({...form,category:event.target.value})}>{["Tekrar Eden Problem","Kurulum","Entegrasyon","MÃ¼ÅŸteri Ä°letiÅŸimi","Saha Operasyonu","EÄŸitim","DiÄŸer"].map(item=><option key={item}>{item}</option>)}</select></Field></div>
      <Field label="Ne Ã¶ÄŸrendik?"><textarea style={{...iStyle,minHeight:80,resize:"vertical"}} value={form.lesson} onChange={event=>setForm({...form,lesson:event.target.value})}/></Field>
      <Field label="Tekrar etmemesi iÃ§in aksiyon"><textarea style={{...iStyle,minHeight:64,resize:"vertical"}} value={form.prevention} onChange={event=>setForm({...form,prevention:event.target.value})}/></Field>
      <div style={{display:"flex",justifyContent:"flex-end"}}><Btn onClick={save}>Kaydet</Btn></div>
    </div>}
    <div style={{display:"grid",gap:9}}>{lessons.map(item=><div key={item.id} style={{background:"#fff",border:"1px solid #E2E8F0",borderLeft:"4px solid #7C3AED",borderRadius:12,padding:14}}><div style={{display:"flex",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}><b>{item.title}</b><span style={{fontSize:9,fontWeight:900,color:"#6D28D9",background:"#F5F3FF",borderRadius:7,padding:"4px 7px"}}>{item.category}</span></div><div style={{fontSize:12,color:"#334155",lineHeight:1.55,whiteSpace:"pre-wrap",marginTop:8}}>{item.lesson}</div>{item.prevention&&<div style={{fontSize:11,color:"#047857",background:"#ECFDF5",borderRadius:9,padding:"8px 10px",marginTop:9}}><b>Ã–nleyici aksiyon:</b> {item.prevention}</div>}<div style={{fontSize:10,color:"#94A3B8",marginTop:8}}>{item.authorName} Â· {new Date(item.createdAt||now()).toLocaleString("tr-TR")}</div>{canEdit&&<button onClick={()=>onChange({lessonsLearned:lessons.filter(lesson=>lesson.id!==item.id)})} style={{marginTop:8,border:0,background:"transparent",color:"#E11D48",fontSize:10,fontWeight:800,cursor:"pointer"}}>Sil</button>}</div>)}</div>
    {!lessons.length&&<div style={{padding:36,textAlign:"center",border:"1px dashed #CBD5E1",borderRadius:12,color:"#94A3B8"}}>Bu proje iÃ§in henÃ¼z Ã¶ÄŸrenilmiÅŸ ders yok.</div>}
  </div>;
}
...
function InvoiceMilestonePanel({project,onChange}) {
  ...
  const save=()=>{
    ...
    onChange({invoiceMilestones:[{id:uid(),...form,title:form.title.trim()||milestone?.name||"Fatura koÅŸulu",createdAt:now()},...rows]});
    setForm({milestoneId:"",title:"",amount:"",conditions:""});
  };
  return <div>
    <div style={{background:"#FFF7ED",border:"1px solid #FED7AA",borderRadius:12,padding:"10px 13px",fontSize:11,color:"#9A3412",marginBottom:12,display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",flexWrap:"wrap"}}><span>Yalnızca yöneticiler görür. Milestone tamamlandığında hangi fatura kaleminin kesilebilir olduğunu takip eder.</span><Btn small variant="secondary" onClick={downloadMonthlyReport}>Aylık Fatura Raporu</Btn></div>
    <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:14,padding:14,marginBottom:13,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:9}}><Field label="Milestone"><select style={iStyle} value={form.milestoneId} onChange={event=>setForm({...form,milestoneId:event.target.value})}><option value="">- SeÃ§ -</option>{(project.milestones||[]).map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><Field label="Fatura Kalemi"><input style={iStyle} value={form.title} onChange={event=>setForm({...form,title:event.target.value})}/></Field><Field label="Tutar / Oran"><input style={iStyle} value={form.amount} onChange={event=>setForm({...form,amount:event.target.value})}/></Field><Field label="KoÅŸul"><input style={iStyle} value={form.conditions} onChange={event=>setForm({...form,conditions:event.target.value})}/></Field><div style={{alignSelf:"end"}}><Btn onClick={save}>Ekle</Btn></div></div>
    <div style={{display:"grid",gap:8}}>{rows.map(row=>{const milestone=(project.milestones||[]).find(item=>item.id===row.milestoneId);const ready=milestone?milestone.tasks?.length>0&&milestone.tasks.every(task=>task.status==="TamamlandÄ±"):false;return <div key={row.id} style={{background:"#fff",border:"1px solid #E2E8F0",borderLeft:`4px solid ${ready?"#059669":"#EA6C00"}`,borderRadius:12,padding:13,display:"flex",justifyContent:"space-between",gap:10}}><div><b>{row.title}</b><div style={{fontSize:11,color:"#64748B",marginTop:4}}>{milestone?.name||"Milestone seçilmedi"} · Termin: {fmt(milestone?.dueDate)} · {row.amount||"Tutar yok"}</div>{row.conditions&&<div style={{fontSize:11,color:"#334155",marginTop:5}}>{row.conditions}</div>}</div><div style={{textAlign:"right"}}><span style={{display:"inline-block",background:ready?"#ECFDF5":"#FFF7ED",color:ready?"#047857":"#C2410C",borderRadius:8,padding:"5px 8px",fontSize:10,fontWeight:900}}>{ready?"Faturalanabilir":"Bekliyor"}</span><button onClick={()=>onChange({invoiceMilestones:rows.filter(item=>item.id!==row.id)})} style={{display:"block",marginTop:7,border:0,background:"transparent",color:"#E11D48",fontSize:10,fontWeight:800,cursor:"pointer"}}>Sil</button></div></div>;})}</div>
  </div>;
}
...
function CommunicationPlanPanel({project,onChange,state,setState,canEdit}) {
  ...
  return <div>
    <div style={{background:"#EEF2FF",color:"#4338CA",borderRadius:11,padding:"10px 13px",fontSize:11,marginBottom:12}}>Şirket kontağı, müşteri kontağı, metod ve sıklık bazında iletişim ritmini tanımlar. Bizim taraftaki kişiye takip görevi otomatik düşer.</div>
    {canEdit&&<div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:14,padding:14,marginBottom:13,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:9}}><Field label="Bizim Taraftaki Kişi"><select style={iStyle} value={form.companyUserId} onChange={event=>setForm({...form,companyUserId:event.target.value})}><option value="">- Seç -</option>{state.people.map(person=><option key={person.id} value={person.id}>{person.name}</option>)}</select></Field><Field label="İletişim Metodu"><select style={iStyle} value={form.method} onChange={event=>setForm({...form,method:event.target.value})}>{["Arama","Mail","Yüz yüze","Online toplantı","WhatsApp","Diğer"].map(item=><option key={item}>{item}</option>)}</select></Field><Field label="Müşteri Kontağı"><select style={iStyle} value={form.customerContactId} onChange={event=>setForm({...form,customerContactId:event.target.value})}><option value="">- Seç -</option>{customerContacts.map(contact=><option key={contact.id} value={contact.id}>{contact.name} {contact.title?`· ${contact.title}`:""}</option>)}</select></Field><Field label="Sıklık"><select style={iStyle} value={form.frequency} onChange={event=>setForm({...form,frequency:event.target.value})}>{["Günlük","Haftalık","İki haftada bir","Aylık","Milestone bazlı","İhtiyaç halinde"].map(item=><option key={item}>{item}</option>)}</select></Field><Field label="Not"><input style={iStyle} value={form.note} onChange={event=>setForm({...form,note:event.target.value})}/></Field><div style={{alignSelf:"end"}}><Btn onClick={save}>Planı Ekle</Btn></div></div>}
    <div style={{display:"grid",gap:8}}>{plans.map(item=><div key={item.id} style={{background:"#fff",border:"1px solid #E2E8F0",borderLeft:"4px solid #4A6CF7",borderRadius:12,padding:13,display:"flex",justifyContent:"space-between",gap:10}}><div><b>{item.companyUserName||state.people.find(person=>person.id===item.companyUserId)?.name}</b><div style={{fontSize:11,color:"#64748B",marginTop:4}}>{item.method} · {item.customerContactName||customerContacts.find(contact=>contact.id===item.customerContactId)?.name||"Müşteri kontağı"} · {item.frequency}</div>{item.note&&<div style={{fontSize:11,color:"#334155",marginTop:5}}>{item.note}</div>}</div>{canEdit&&<button onClick={()=>onChange({communicationPlans:plans.filter(plan=>plan.id!==item.id)})} style={{border:0,background:"transparent",color:"#E11D48",fontSize:10,fontWeight:800,cursor:"pointer"}}>Sil</button>}</div>)}</div>
    {!plans.length&&<div style={{padding:36,textAlign:"center",border:"1px dashed #CBD5E1",borderRadius:12,color:"#94A3B8"}}>İletişim planı bulunmuyor.</div>}
  </div>;
}
...
export function ProjectSetupPanel({project,onChange,canEdit,customerView=false,state,setState,currentUser,isAdmin,authHeaders,updateApiStateVersion,AIWorkspaceComponent=()=>null}) {
  ...
  const legacyCustomerContacts=(project.customerContacts||[]).map(item=>({...item,side:item.side||"MÃ¼ÅŸteri",raci:item.raci||"I",scope:item.scope||"Eski mÃ¼ÅŸteri kontaÄŸÄ±",legacyCustomerContact:true}));
  ...
  return <div style={{flex:1,overflow:"auto",padding:"clamp(14px,3vw,24px)"}}>
    ...
    {section==="raci"&&<div>
      <div style={{background:"#EEF2FF",color:"#4338CA",borderRadius:11,padding:"10px 13px",fontSize:11,marginBottom:12}}><b>R</b> yapan, <b>A</b> nihai hesap veren, <b>C</b> gÃ¶rÃ¼ÅŸÃ¼ alÄ±nan, <b>I</b> bilgilendirilen. Her kapsam iÃ§in tek bir A belirlenmesi Ã¶nerilir.</div>
      <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",background:"#fff",fontSize:11}}><thead><tr>{["Taraf","Ad Soyad","Unvan / Departman","RACI","Sorumluluk Kapsam\u0131","Kontak",""].map(x=><th key={x} style={{textAlign:"left",padding:10,background:"#F8FAFC",borderBottom:"1px solid #E2E8F0"}}>{x}</th>)}</tr></thead><tbody>{raciRows.map(item=><tr key={`${item.legacyCustomerContact?"legacy":"raci"}-${item.id}`}><td style={{padding:9}}>{item.side}{item.legacyCustomerContact&&<span style={{display:"block",fontSize:9,color:"#EA6C00",fontWeight:800}}>Eski kontak</span>}</td><td style={{padding:9,fontWeight:750}}>{item.name}</td><td style={{padding:9}}>{item.title}{item.department?` / ${item.department}`:""}</td><td style={{padding:9,color:"#4F46E5",fontWeight:850}}>{item.raci}</td><td style={{padding:9}}>{item.scope}</td><td style={{padding:9}}>{item.email}<br/>{item.phone}</td><td>{canEdit&&<button onClick={()=>deleteRaciRow(item)} style={{border:0,background:"transparent",color:"#E11D48",cursor:"pointer"}}>Sil</button>}</td></tr>)}</tbody></table></div>
      {canEdit&&<div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:13,padding:13,marginTop:12,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:8}}><select style={iStyle} value={contact.side} onChange={e=>setContact({...contact,side:e.target.value})}><option value="Bizim Taraf">{tenantName}</option><option value="Müşteri">{projectPartyName}</option><option value="Partner/Tedarikçi">Partner / Tedarikçi</option></select>{["name","title","company","department","email","phone","scope"].map(key=><input key={key} style={iStyle} value={contact[key]} onChange={e=>setContact({...contact,[key]:e.target.value})} placeholder={({name:"Ad Soyad",title:"Unvan",company:"Şirket",department:"Departman",email:"E-posta",phone:"Telefon",scope:"Sorumluluk kapsamı"})[key]}/>)}<select style={iStyle} value={contact.raci} onChange={e=>setContact({...contact,raci:e.target.value})}>{["R","A","C","I"].map(x=><option key={x}>{x}</option>)}</select><Btn onClick={addContact}>Kontağı Ekle</Btn></div>}
    </div>}
    ...
    {section==="documents"&&<div>
      <div style={{background:"#F0F9FF",color:"#0369A1",borderRadius:11,padding:"10px 13px",fontSize:11,marginBottom:12}}>DosyanÄ±n kendisi yerine ÅŸimdilik OneDrive baÄŸlantÄ±sÄ± ve metadata saklanÄ±r. Microsoft Graph baÄŸlantÄ±sÄ± eklendiÄŸinde aynÄ± kayÄ±t modeli doÄŸrudan yÃ¼klemeyi destekleyecek.</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:9}}>{(project.documents||[]).map(item=><div key={item.id} style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:12,padding:13}}><div style={{display:"flex",justifyContent:"space-between"}}><b>{item.name}</b><span style={{fontSize:9}}>v{item.version}</span></div><div style={{fontSize:11,color:"#64748B",marginTop:5}}>{item.purpose}</div><div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:7}}>{(item.tags||[]).map(tag=><span key={tag} style={{background:"#EEF2FF",color:"#4338CA",borderRadius:6,padding:"2px 6px",fontSize:9}}>{tag}</span>)}</div><div style={{display:"flex",gap:9,marginTop:9}}>{item.url&&<a href={item.url} target="_blank" rel="noreferrer" style={{fontSize:10,fontWeight:800,color:"#2563EB"}}>OneDrive'da AÃ§</a>}{canEdit&&<button onClick={()=>onChange({documents:(project.documents||[]).filter(x=>x.id!==item.id)})} style={{border:0,background:"transparent",color:"#E11D48",fontSize:10,cursor:"pointer"}}>Sil</button>}</div></div>)}</div>
      {canEdit&&<div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:13,padding:13,marginTop:12,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:8}}>{["name","purpose","tags","url","owner","version"].map(key=><input key={key} style={iStyle} value={document[key]} onChange={e=>setDocument({...document,[key]:e.target.value})} placeholder={({name:"DokÃ¼man adÄ±",purpose:"AmaÃ§ / kategori",tags:"Etiketler (virgÃ¼llÃ¼)",url:"OneDrive linki",owner:"DokÃ¼man sahibi",version:"Versiyon"})[key]}/>)}<Btn onClick={addDocument}>DokÃ¼man Ekle</Btn></div>}
    </div>}
    {section==="automation"&&<div>
      ...
      {canEdit&&<div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:13,padding:13,marginTop:12,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:8}}><input style={iStyle} value={schedule.name} onChange={e=>setSchedule({...schedule,name:e.target.value})} placeholder="Plan adÄ±"/><select style={iStyle} value={schedule.reportType} onChange={e=>setSchedule({...schedule,reportType:e.target.value})}><option value="project_status">Proje Durum Raporu</option><option value="jira_newsletter">Jira Done Newsletter</option></select><input style={iStyle} value={schedule.recipients} onChange={e=>setSchedule({...schedule,recipients:e.target.value})} placeholder="AlÄ±cÄ± e-postalarÄ±"/><select style={iStyle} value={schedule.frequency} onChange={e=>setSchedule({...schedule,frequency:e.target.value})}><option value="weekly">HaftalÄ±k</option><option value="monthly">AylÄ±k</option></select>{schedule.frequency==="weekly"&&<select style={iStyle} value={schedule.weekday} onChange={e=>setSchedule({...schedule,weekday:e.target.value})}>{["Pazar","Pazartesi","SalÄ±","Ã‡arÅŸamba","PerÅŸembe","Cuma","Cumartesi"].map((day,index)=><option key={day} value={index}>{day}</option>)}</select>}<input type="time" title="Europe/Istanbul saat dilimi" style={iStyle} value={schedule.time} onChange={e=>setSchedule({...schedule,time:e.target.value})}/><Btn onClick={addSchedule}>Planla</Btn></div>}
    </div>}
...
export function ResponsibilitySummary({project,selected=[],onChange}) {
  ...
  return <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:13,padding:13,marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"center",marginBottom:9}}><div><div style={{fontSize:12,fontWeight:850}}>Kalan Ä°ÅŸin Sorumluluk DaÄŸÄ±lÄ±mÄ±</div><div style={{fontSize:9,color:"#94A3B8",marginTop:2}}>Bir veya birden fazla ekibe tÄ±klayarak gÃ¶revleri filtreleyin.</div></div>{selected.length>0&&<button onClick={()=>onChange([])} style={{border:0,background:"#FFF1F2",color:"#BE123C",borderRadius:7,padding:"5px 8px",fontSize:9,fontWeight:800,cursor:"pointer"}}>Filtreyi temizle</button>}</div>{items.length?<div style={{display:"grid",gap:7}}>{items.map((item,index)=><ResponsibilityFilterRow key={item.group} item={item} index={index} active={selected.includes(item.group)} onClick={()=>toggle(item.group)}/>)}</div>:<div style={{fontSize:11,color:"#059669"}}>Kalan gÃ¶rev bulunmuyor.</div>}</div>;
}
...
export function ProjectNotesPanel({ project, currentUser, state, setState, isAdmin, canManage }) {
  ...
  const emptyAccess={name:"",url:"",username:"",status:"HazÄ±r",routing:"",secretReference:"",note:""};
  ...
  return <div style={{ flex:1, overflow:"auto", padding:"clamp(14px, 3vw, 24px)" }}>
    ...
    {section==="access"&&<div>
      <div style={{background:"#FFF7ED",border:"1px solid #FED7AA",borderRadius:10,padding:"10px 12px",fontSize:11,color:"#9A3412",marginBottom:13}}>GÃ¼venlik nedeniyle gerÃ§ek parola burada tutulmaz. â€œParola kasasÄ± referansÄ±â€ alanÄ±na 1Password, Bitwarden veya kurum kasasÄ±ndaki kayÄ±t adÄ±nÄ± yazÄ±n.</div>
      {(isAdmin||canManage)&&<div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:13,padding:15,marginBottom:14}}><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:9}}>{[["name","Sistem / Sunucu"],["url","BaÄŸlantÄ± Adresi"],["username","KullanÄ±cÄ± AdÄ±"],["routing","VPN / YÃ¶nlendirme"],["secretReference","Parola KasasÄ± ReferansÄ±"]].map(([key,label])=><Field key={key} label={label}><input style={iStyle} value={accessForm[key]} onChange={event=>setAccessForm(current=>({...current,[key]:event.target.value}))}/></Field>)}<Field label="Durum"><select style={iStyle} value={accessForm.status} onChange={event=>setAccessForm(current=>({...current,status:event.target.value}))}>{["HazÄ±r","Test Bekliyor","EriÅŸim Yok","SÃ¼resi Doldu"].map(status=><option key={status}>{status}</option>)}</select></Field></div><Field label="Not"><input style={iStyle} value={accessForm.note} onChange={event=>setAccessForm(current=>({...current,note:event.target.value}))}/></Field><div style={{display:"flex",justifyContent:"flex-end"}}><Btn onClick={addAccess}>EriÅŸim KaydÄ± Ekle</Btn></div></div>}
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(300px,100%),1fr))",gap:10}}>{accessItems.map(item=><div key={item.id} style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:12,padding:14}}><div style={{display:"flex",justifyContent:"space-between",gap:8}}><b style={{fontSize:13}}>{item.name}</b><span style={{fontSize:9,fontWeight:850,color:item.status==="HazÄ±r"?"#047857":"#C2410C",background:item.status==="HazÄ±r"?"#ECFDF5":"#FFF7ED",padding:"3px 6px",borderRadius:7}}>{item.status}</span></div><div style={{fontSize:11,color:"#64748B",lineHeight:1.7,marginTop:8}}>{item.url&&<div>Adres: <b>{item.url}</b></div>}{item.username&&<div>KullanÄ±cÄ±: <b>{item.username}</b></div>}{item.routing&&<div>YÃ¶nlendirme: {item.routing}</div>}{item.secretReference&&<div>Kasa kaydÄ±: <b>{item.secretReference}</b></div>}{item.note&&<div>Not: {item.note}</div>}</div>{(isAdmin||canManage)&&<button onClick={()=>confirm("EriÅŸim kaydÄ± silinsin mi?")&&saveAccessItems(accessItems.filter(entry=>entry.id!==item.id))} style={{marginTop:8,border:0,background:"transparent",color:"#E11D48",fontSize:10,cursor:"pointer"}}>Sil</button>}</div>)}</div>
      {!accessItems.length&&<div style={{padding:35,textAlign:"center",border:"1px dashed #CBD5E1",borderRadius:12,color:"#94A3B8"}}>Uzaktan eriÅŸim kaydÄ± bulunmuyor.</div>}
    </div>}
    {section!=="access"&&<>
    <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
    {/* Left: shared notes */}
    <div style={{ flex:"1 1 340px", minWidth:280 }}>
      <div style={{ fontWeight:800, fontSize:15, marginBottom:14 }}>{section==="notes"?"Proje NotlarÄ±":"Proje To-Do"}</div>

      {section==="notes"&&<div style={{ background:"#fff", borderRadius:12, border:"1.5px solid #E2E8F0", padding:"16px", marginBottom:16 }}>
        <div style={{ fontWeight:700, fontSize:13, marginBottom:8, color:"#1E293B" }}>PaylaÅŸÄ±lan Notlar</div>
        {isAdmin
          ? <textarea value={editNote} onChange={e=>{setEditNote(e.target.value);save({shared:e.target.value});}} placeholder="Proje genelinde paylaÅŸÄ±lacak not, karar, Ã¶nemli bilgi..." style={{ width:"100%", minHeight:120, padding:"9px", borderRadius:8, border:"1.5px solid #E2E8F0", fontSize:12, fontFamily:"inherit", resize:"vertical", boxSizing:"border-box", background:"#FAFBFC", outline:"none", lineHeight:1.6 }} />
          : <div style={{ minHeight:80, fontSize:12, color: projNotes.shared?"#1E293B":"#94A3B8", lineHeight:1.7, whiteSpace:"pre-wrap" }}>{projNotes.shared||"HenÃ¼z not girilmemiÅŸ."}</div>
        }
      </div>}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
          {projNotes.items.map(item=><div key={item.id} style={{ display:"flex", alignItems:"flex-start", gap:8, padding:"9px 11px", background:"#F8FAFC", borderRadius:8, border:"1.5px solid #E2E8F0" }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:10, color:"#94A3B8", marginTop:2 }}>{item.author} Â· {new Date(item.ts).toLocaleDateString("tr-TR")}</div>
            </div>
    ...
    <div style={{ flex:"1 1 300px", minWidth:0 }}>
      <div style={{ fontWeight:800, fontSize:15, marginBottom:14 }}>{section==="notes"?"Ekip NotlarÄ±":"KiÅŸisel To-Do BaÄŸlantÄ±larÄ±m"}</div>
      {section==="todos"&&<div style={{ background:"#fff", borderRadius:12, border:"1.5px solid #E2E8F0", padding:"16px" }}>
        <div style={{ fontWeight:700, fontSize:13, marginBottom:10, color:"#64748B" }}>Bu projeye baÄŸlÄ± todo larÄ±</div>
        {linkedTodos.length===0&&<div style={{ fontSize:12, color:"#94A3B8" }}>HenÃ¼z baÄŸlÄ± kiÅŸisel todo yok.<br/>GÃ¶revlerim sayfasÄ±ndan todo eklerken proje seÃ§in.</div>}
        <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
          {linkedTodos.map(t=><div key={t.id} style={{ display:"flex", alignItems:"flex-start", gap:8, padding:"9px 11px", background:"#F8FAFC", borderRadius:8, border:"1.5px solid #E2E8F0" }}>
            ...
            {t.done&&<span style={{ fontSize:11, color:"#059669" }}>âœ“</span>}
          </div>)}
        </div>
      </div>}
      ...
      {section==="notes"&&isAdmin&&<div style={{ background:"#fff", borderRadius:12, border:"1.5px solid #E2E8F0", padding:"16px" }}>
        <div style={{ fontWeight:700, fontSize:13, marginBottom:10, color:"#64748B" }}>Ekip Serbest NotlarÄ±</div>
        {state.people.map(p=>{
