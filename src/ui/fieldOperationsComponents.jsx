...
function FieldPlanPage({
  ...
  return <div style={{padding:"22px clamp(14px,4vw,28px)",flex:1,overflow:"auto"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap",marginBottom:18}}>
      <div><h2 style={{margin:0,fontSize:20,fontWeight:800,display:"flex",alignItems:"center",gap:8}}><Icon name="calendar" size={21}/>HaftalÄ±k Ã‡alÄ±ÅŸma PlanÄ±m</h2><p style={{margin:"4px 0 0",fontSize:12,color:"#64748B"}}>Saha ziyaretlerini ve proje bazlÄ± uzaktan Ã§alÄ±ÅŸmalarÄ± haftalÄ±k planlayÄ±n.</p></div>
    ...
    {showForm&&<div style={{background:"#fff",border:"1.5px solid #E2E8F0",borderRadius:14,padding:16,marginBottom:16,boxShadow:"0 8px 24px rgba(15,23,42,.05)"}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,alignItems:"end"}}>
        ...
        <Field label="Ã‡alÄ±ÅŸma TÃ¼rÃ¼"><select style={iStyle} value={form.workType} onChange={e=>setForm({...form,workType:e.target.value})}><option value="field">Saha Ziyareti</option><option value="remote">Uzaktan Ã‡alÄ±ÅŸma</option></select></Field>
        <Field label="MÃ¼ÅŸteri / Proje"><select style={iStyle} value={form.projectId} onChange={e=>setForm({...form,projectId:e.target.value})}><option value="">Proje seÃ§in</option>{state.projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>
        <Field label="Tarih"><input type="date" style={iStyle} value={form.date} onChange={e=>setForm({...form,date:e.target.value})}/></Field>
        <Field label="BaÅŸlangÄ±Ã§"><input type="time" style={iStyle} value={form.startTime} onChange={e=>setForm({...form,startTime:e.target.value})}/></Field>
        <Field label="BitiÅŸ"><input type="time" style={iStyle} value={form.endTime} onChange={e=>setForm({...form,endTime:e.target.value})}/></Field>
      </div>
      {form.workType==="field"&&<div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",marginTop:8}}><label style={{display:"flex",alignItems:"center",gap:7,fontSize:12,fontWeight:800,color:"#334155"}}><input type="checkbox" checked={Boolean(form.accommodationRequired)} onChange={e=>setForm({...form,accommodationRequired:e.target.checked})}/> Konaklama maliyeti var</label>{form.accommodationRequired&&<Field label="Konaklama Gün Sayısı"><input type="number" min="1" step="1" style={{...iStyle,width:120}} value={form.accommodationDays||1} onChange={e=>setForm({...form,accommodationDays:e.target.value})}/></Field>}</div>}      <Field label="Plan Notu"><textarea style={{...iStyle,minHeight:70,resize:"vertical"}} value={form.note} onChange={e=>setForm({...form,note:e.target.value})} placeholder={form.workType==="remote"?"Uzaktan yapÄ±lacak Ã§alÄ±ÅŸma, hedef ve beklenen Ã§Ä±ktÄ±...":"Ziyaret amacÄ±, gÃ¶rÃ¼ÅŸÃ¼lecek kiÅŸiler veya hazÄ±rlÄ±k notu..."}/></Field>
      <div style={{display:"flex",justifyContent:"flex-end"}}><Btn disabled={!form.projectId||!form.date} onClick={save}>{editingId?"DeÄŸiÅŸiklikleri Kaydet":"PlanÄ± Kaydet"}</Btn></div>
    </div>}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:12}}>
      {isAdmin?<div style={{display:"flex",gap:7,flexWrap:"wrap"}}><div style={{display:"flex",background:"#E2E8F0",padding:3,borderRadius:10}}>{[["mine","Benim PlanÄ±m"],["team","TÃ¼m Ekip"]].map(([id,label])=><button key={id} onClick={()=>setScope(id)} style={{border:0,borderRadius:8,padding:"7px 12px",cursor:"pointer",fontWeight:700,fontSize:12,background:scope===id?"#fff":"transparent",color:scope===id?"#4A6CF7":"#64748B",boxShadow:scope===id?"0 2px 6px #0f172a14":"none"}}>{label}</button>)}</div>{scope==="team"&&<select style={{...iStyle,width:190}} value={personFilter} onChange={e=>setPersonFilter(e.target.value)}><option value="">TÃ¼m kiÅŸiler</option>{state.people.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>}<select style={{...iStyle,width:210}} value={projectFilter} onChange={e=>setProjectFilter(e.target.value)}><option value="all">TÃ¼m projeler</option>{state.projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>:<span style={{fontSize:12,fontWeight:700,color:"#4A6CF7"}}>Benim PlanÄ±m</span>}
      <div style={{display:"flex",alignItems:"center",gap:6}}><button onClick={()=>setWeekOffset(v=>v-1)} style={{border:0,borderRadius:8,padding:"7px 10px",background:"#fff",cursor:"pointer",color:"#64748B"}}>â€¹</button><button onClick={()=>setWeekOffset(0)} style={{border:0,borderRadius:8,padding:"7px 11px",background:"#F1F5FF",cursor:"pointer",fontWeight:700,color:"#4A6CF7"}}>{monthLabel}</button><button onClick={()=>setWeekOffset(v=>v+1)} style={{border:0,borderRadius:8,padding:"7px 10px",background:"#fff",cursor:"pointer",color:"#64748B"}}>â€º</button></div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(165px,1fr))",gap:9}}>
      {days.map(d=>{const key=dateKey(d);const dayPlans=visible.filter(p=>p.date===key).sort((a,b)=>(a.startTime||"").localeCompare(b.startTime||""));const today=key===todayStr();return <div key={key} style={{background:today?"#F1F5FF":"#fff",border:`1.5px solid ${today?"#A5B4FC":"#E2E8F0"}`,borderRadius:13,minHeight:190,padding:11}}>
        ...
        {dayPlans.map(plan=>{const project=state.projects.find(p=>p.id===plan.projectId);const person=state.people.find(p=>p.id===plan.userId);const remote=plan.workType==="remote";return <div key={plan.id} onClick={()=>openForm(key,plan)} style={{background:"#fff",border:`1px solid ${remote?"#C4B5FD":project?.color||"#CBD5E1"}55`,borderLeft:`4px solid ${remote?"#7C3AED":project?.color||"#4A6CF7"}`,borderRadius:9,padding:"9px 8px",marginBottom:7,boxShadow:"0 2px 8px #0f172a0c",cursor:"pointer"}}>
          ...
          {plan.status==="completed"&&<div style={{fontSize:9,fontWeight:800,color:"#059669",background:"#ECFDF5",borderRadius:6,padding:"3px 6px",marginTop:6,display:"inline-block"}}>GERÃ‡EKLEÅTÄ° Â· {fieldPlanHours(plan)} SA</div>}
          {(isAdmin||plan.userId===currentUser.id)&&<div style={{display:"flex",gap:8,marginTop:6,flexWrap:"wrap"}}>{plan.status!=="completed"&&<button onClick={e=>{e.stopPropagation();setVisitPlan(plan);}} style={{border:0,background:"transparent",color:remote?"#7C3AED":"#059669",fontSize:10,fontWeight:800,cursor:"pointer",padding:0}}>{remote?"Ã‡alÄ±ÅŸmayÄ± Tamamla":"Ziyareti Tamamla"}</button>}<button onClick={e=>{e.stopPropagation();openForm(key,plan);}} style={{border:0,background:"transparent",color:"#4A6CF7",fontSize:10,cursor:"pointer",padding:0}}>DÃ¼zenle</button><button onClick={e=>{e.stopPropagation();if(confirm("Plan silinsin mi?"))remove(plan.id);}} style={{border:0,background:"transparent",color:"#E11D48",fontSize:10,cursor:"pointer",padding:0}}>Sil</button></div>}
        </div>})}
...
export function FieldVisitModal({plan,project,currentUser,onClose,onSave}) {
  ...
  const save=()=>{
    ...
    if(hours<=0){alert("GeÃ§erli bir efor sÃ¼resi girin.");return;}
    if(!form.visitNotes.trim()){alert(remote?"Uzaktan Ã§alÄ±ÅŸmada yapÄ±lanlarÄ± yazÄ±n.":"Ziyarette yapÄ±lanlarÄ± yazÄ±n.");return;}
  ...
  return <Modal title={`${remote?"Uzaktan Ã‡alÄ±ÅŸma":"Saha Ziyareti"} Â· ${project?.name||"Proje"}`} onClose={onClose} wide>
    <div style={{background:"#ECFDF5",border:"1px solid #A7F3D0",borderRadius:11,padding:"11px 13px",marginBottom:14,fontSize:11,color:"#047857"}}><b>{fmt(plan.date)}</b> Â· Planlanan {plan.startTime} - {plan.endTime}{suggested?` Â· ${suggested} saat`:""}</div>
    <div className="visit-time-grid" style={{display:"grid",gridTemplateColumns:`repeat(${remote?3:4},minmax(0,1fr))`,gap:10}}>
      <Field label="GerÃ§ek BaÅŸlangÄ±Ã§"><input type="time" style={iStyle} value={form.actualStartTime} onChange={e=>update("actualStartTime",e.target.value)}/></Field>
      <Field label="GerÃ§ek BitiÅŸ"><input type="time" style={iStyle} value={form.actualEndTime} onChange={e=>update("actualEndTime",e.target.value)}/></Field>
      ...
      {!remote&&<Field label="GidiÅŸ-DÃ¶nÃ¼ÅŸ KM"><input type="number" min="0" step="1" style={iStyle} value={form.roundTripKm} onChange={e=>update("roundTripKm",e.target.value)} placeholder="Ã¶rn. 180"/></Field>}
    </div>
    {!remote&&tripKm>0&&<div style={{fontSize:11,color:"#64748B",background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:9,padding:"8px 10px",marginTop:8,marginBottom:4}}>YakÄ±t maliyeti tahmini: <b>${tripCost.usdAmount}</b> / <b>{tripCost.tryAmount} TL</b> Â· {costSettings.fuelConsumptionLtPer100Km} L/100km, {costSettings.fuelTryPerLt} TL/L, kur {costSettings.usdTry}</div>}
    <Field label={remote?"Uzaktan Ã‡alÄ±ÅŸmada YapÄ±lanlar":"Ziyarette YapÄ±lanlar"}><textarea style={{...iStyle,minHeight:115,resize:"vertical",lineHeight:1.5}} value={form.visitNotes} onChange={e=>update("visitNotes",e.target.value)} placeholder={remote?"YapÄ±lan analiz, kontrol, geliÅŸtirme, gÃ¶rÃ¼ÅŸme ve sonraki aksiyonlar...":"GÃ¶rÃ¼ÅŸÃ¼len konular, yapÄ±lan kontroller, alÄ±nan kararlar ve sonraki aksiyonlar..."}/></Field>
    <div style={{display:"flex",justifyContent:"flex-end",gap:8}}><Btn variant="ghost" onClick={onClose}>Ä°ptal</Btn><Btn onClick={save}>{remote?"Ã‡alÄ±ÅŸmayÄ±":"Ziyareti"} Tamamla ve Eforu Kaydet</Btn></div>
  </Modal>;
}
...
function FieldVisitsPage({
  ...
  return <div style={{padding:"clamp(16px,4vw,28px)",flex:1,overflow:"auto"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",gap:12,flexWrap:"wrap",marginBottom:18}}>
      <div><h2 style={{margin:0,fontSize:21,display:"flex",alignItems:"center",gap:8}}><Icon name="calendar" size={21}/>GerÃ§ekleÅŸen Ã‡alÄ±ÅŸmalar</h2><p style={{margin:"4px 0 0",fontSize:12,color:"#64748B"}}>Saha ziyaretleri ve uzaktan Ã§alÄ±ÅŸmalarÄ±n notlarÄ± ile proje eforlarÄ±.</p></div>
      <div style={{display:"flex",gap:7,background:"#E2E8F0",padding:3,borderRadius:10}}>{[["mine","Benim Ziyaretlerim"],["responsible",isAdmin?"TÃ¼m Ekip":"Sorumlu Projeler"]].map(([id,label])=><button key={id} onClick={()=>{setScope(id);setProjectFilter("all");setPersonFilter("");}} style={{border:0,borderRadius:8,padding:"7px 11px",cursor:"pointer",fontSize:11,fontWeight:800,background:scope===id?"#fff":"transparent",color:scope===id?"#4A6CF7":"#64748B"}}>{label}</button>)}</div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:9,marginBottom:14}}>{[["Saha Ziyareti",visible.filter(plan=>(plan.workType||"field")==="field").length,"#059669"],["Uzaktan Ã‡alÄ±ÅŸma",visible.filter(plan=>plan.workType==="remote").length,"#7C3AED"],["Toplam Efor",`${hours} sa`,"#0369A1"],["Ã‡alÄ±ÅŸÄ±lan Proje",new Set(visible.map(plan=>plan.projectId)).size,"#EA6C00"]].map(([label,value,color])=><div key={label} style={{background:"#fff",border:"1px solid #E2E8F0",borderTop:`3px solid ${color}`,borderRadius:12,padding:13}}><div style={{fontSize:10,color:"#64748B"}}>{label}</div><div style={{fontSize:23,fontWeight:900,color,marginTop:3}}>{value}</div></div>)}</div>
    <div style={{display:"flex",gap:9,flexWrap:"wrap",marginBottom:14}}>
      <select style={{...iStyle,width:"auto",minWidth:180}} value={typeFilter} onChange={e=>setTypeFilter(e.target.value)}><option value="all">TÃ¼m Ã‡alÄ±ÅŸma TÃ¼rleri</option><option value="field">Saha Ziyaretleri</option><option value="remote">Uzaktan Ã‡alÄ±ÅŸmalar</option></select>
      <select style={{...iStyle,width:"auto",minWidth:220}} value={projectFilter} onChange={e=>setProjectFilter(e.target.value)}><option value="all">TÃ¼m Projeler</option>{projects.map(project=><option key={project.id} value={project.id}>{project.name}</option>)}</select>
      {(isAdmin||scope==="responsible")&&<select style={{...iStyle,width:"auto",minWidth:190}} value={personFilter} onChange={e=>setPersonFilter(e.target.value)}><option value="">TÃ¼m KiÅŸiler</option>{state.people.map(person=><option key={person.id} value={person.id}>{person.name}</option>)}</select>}
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(320px,100%),1fr))",gap:11}}>
      {visible.map(plan=>{const project=state.projects.find(item=>item.id===plan.projectId);const person=state.people.find(item=>item.id===plan.userId);const canEdit=isAdmin||plan.userId===currentUser.id;const remote=plan.workType==="remote";return <div key={plan.id} style={{background:"#fff",border:"1px solid #E2E8F0",borderTop:`4px solid ${remote?"#7C3AED":project?.color||"#4A6CF7"}`,borderRadius:14,padding:15,boxShadow:"0 4px 14px rgba(15,23,42,.04)"}}>
        ...
        <div style={{fontSize:9,fontWeight:850,color:remote?"#6D28D9":"#047857",marginTop:9}}>{remote?"UZAKTAN Ã‡ALIÅMA":"SAHA ZÄ°YARETÄ°"}</div><div style={{fontSize:11,lineHeight:1.55,color:"#334155",whiteSpace:"pre-wrap",background:"#F8FAFC",borderRadius:9,padding:"9px 10px",marginTop:6}}>{plan.visitNotes||(remote?"Ã‡alÄ±ÅŸma notu girilmedi.":"Ziyaret notu girilmedi.")}</div>
        {canEdit&&<button onClick={()=>setEditing(plan)} style={{marginTop:9,border:0,background:"transparent",color:"#4A6CF7",fontSize:10,fontWeight:800,cursor:"pointer",padding:0}}>Ã‡alÄ±ÅŸma ve eforu dÃ¼zenle</button>}
      </div>})}
    </div>
    {!visible.length&&<div style={{padding:45,textAlign:"center",border:"1.5px dashed #CBD5E1",borderRadius:13,color:"#94A3B8",background:"#fff"}}>Bu kapsamda gerÃ§ekleÅŸmiÅŸ Ã§alÄ±ÅŸma bulunmuyor.</div>}
...
export function FieldOperationsPage({
  ...
  return <div style={{flex:1,overflow:"auto",background:"#F8FAFC"}}>
    <div style={{position:"sticky",top:0,zIndex:5,display:"flex",gap:7,padding:"12px clamp(16px,4vw,28px)",background:"#fff",borderBottom:"1px solid #E2E8F0"}}>
      {[["plan","calendar","HaftalÄ±k Plan"],["visits","activity","GerÃ§ekleÅŸen Ã‡alÄ±ÅŸmalar"]].map(([id,icon,label])=><button key={id} onClick={()=>setSection(id)} style={{border:0,borderRadius:9,padding:"8px 13px",background:section===id?"#0F766E":"#F1F5F9",color:section===id?"#fff":"#64748B",fontSize:12,fontWeight:800,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:6}}><Icon name={icon} size={14}/>{label}</button>)}
    </div>
