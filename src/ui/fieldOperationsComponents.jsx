...
function FieldPlanPage({
  ...
  return <div className="theme-work-page fieldops-theme-page field-plan-page" style={{padding:"22px clamp(14px,4vw,28px)",flex:1,overflow:"auto"}}>
    ...
    <div className="unified-filter-row fieldops-filter-row fieldops-plan-filter-row" style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:12}}>
      {isAdmin?<div className="segmented-control compact-filter-segment fieldops-scope-switch" role="group" aria-label="Saha planı kapsamı">{[["mine","user","Benim Planım"],["team","people","Tüm Ekip"]].map(([id,icon,label])=><button key={id} title={label} aria-label={label} onClick={()=>setScope(id)} className={scope===id?"is-active":""}><Icon name={icon} size={15}/><span>{label}</span></button>)}</div>:<span className="ui-chip ui-chip-accent">Benim Planım</span>}
      <div className="fieldops-combined-selects">
        {isAdmin&&scope==="team"&&<label className="fieldops-compact-select" title={selectedPersonName||"Tüm kişiler"}><Icon name="people" size={14}/><select value={personFilter} onChange={e=>setPersonFilter(e.target.value)} aria-label="Kişi filtresi"><option value="">Tüm kişiler</option>{state.people.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>}
        <label className="fieldops-compact-select" title={selectedProjectName||"Tüm projeler"}><Icon name="projects" size={14}/><select value={projectFilter} onChange={e=>setProjectFilter(e.target.value)} aria-label="Proje filtresi"><option value="all">Tüm projeler</option>{state.projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
      </div>
...
function FieldVisitsPage({
  ...
  return <div className="theme-work-page fieldops-theme-page field-visits-page" style={{padding:"clamp(16px,4vw,28px)",flex:1,overflow:"auto"}}>
    ...
    <div className="unified-filter-row fieldops-filter-row fieldops-visits-filter-row" style={{display:"flex",gap:9,flexWrap:"wrap",marginBottom:14}}>
      <div className="fieldops-combined-selects">
        <label className="fieldops-compact-select" title={selectedTypeLabel}><Icon name="activity" size={14}/><select value={typeFilter} onChange={e=>setTypeFilter(e.target.value)} aria-label="Çalışma türü filtresi"><option value="all">Tüm türler</option><option value="field">Saha</option><option value="remote">Uzaktan</option></select></label>
        <label className="fieldops-compact-select" title={selectedProjectName||"Tüm projeler"}><Icon name="projects" size={14}/><select value={projectFilter} onChange={e=>setProjectFilter(e.target.value)} aria-label="Proje filtresi"><option value="all">Tüm projeler</option>{projects.map(project=><option key={project.id} value={project.id}>{project.name}</option>)}</select></label>
        {(isAdmin||scope==="responsible")&&<label className="fieldops-compact-select" title={selectedPersonName||"Tüm kişiler"}><Icon name="people" size={14}/><select value={personFilter} onChange={e=>setPersonFilter(e.target.value)} aria-label="Kişi filtresi"><option value="">Tüm kişiler</option>{state.people.map(person=><option key={person.id} value={person.id}>{person.name}</option>)}</select></label>}
      </div>
    </div>
