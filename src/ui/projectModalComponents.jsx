...
export function MilestoneModal({ title, initial, onClose, onSave, waitOptions = [] }) {
  ...
  return <Modal title={title} onClose={onClose}>
    ...
    <div style={{background:"var(--surface-soft)",borderRadius:9,padding:"9px 11px",fontSize:11,color:"#64748B",marginBottom:13}}>Milestone durumu içindeki görevlerin durumuna göre otomatik hesaplanır.</div>
    ...
    <Modal title="Ekip Üyesi Ekle" onClose={onClose}>
      ...
      <Field label="Yetki">
        <div style={{ display:"flex", gap:10 }}>
          <div onClick={()=>setIsAdmin(false)} style={{ flex:1, padding:"10px", borderRadius:8, border:`1.5px solid ${!isAdmin?"#4A6CF7":"#E2E8F0"}`, cursor:"pointer", textAlign:"center", background:!isAdmin?"var(--accent-ink)":"#fff", fontSize:12, fontWeight:600, color:!isAdmin?"#4A6CF7":"#64748B" }}>
            Ekip Üyesi
          </div>
          <div onClick={()=>setIsAdmin(true)} style={{ flex:1, padding:"10px", borderRadius:8, border:`1.5px solid ${isAdmin?"#4A6CF7":"#E2E8F0"}`, cursor:"pointer", textAlign:"center", background:isAdmin?"var(--accent-ink)":"#fff", fontSize:12, fontWeight:600, color:isAdmin?"#4A6CF7":"#64748B" }}>
            Yönetici
          </div>
        </div>
      </Field>
