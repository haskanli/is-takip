...
const LOG_META = {
  task_done: { icon: "âœ“", color: "#059669", bg: "#ECFDF5", label: "TamamlandÄ±" },
  task_add: { icon: "+", color: "#4A6CF7", bg: "#F1F5FF", label: "GÃ¶rev Eklendi" },
  task_delete: { icon: "Ã—", color: "#E11D48", bg: "#FFF1F2", label: "GÃ¶rev Silindi" },
  status_change: { icon: "â†»", color: "#EA6C00", bg: "#FFF7ED", label: "Durum DeÄŸiÅŸti" },
  milestone_add: { icon: "â—†", color: "#7C3AED", bg: "#F5F3FF", label: "Milestone" },
  project_create: { icon: "â–¦", color: "#0EA5E9", bg: "#F0F9FF", label: "Proje" },
  risk_add: { icon: "!", color: "#E11D48", bg: "#FFF1F2", label: "Risk" },
  import: { icon: "â¬†", color: "#64748B", bg: "#F8FAFC", label: "Import" },
  person_add: { icon: "â—", color: "#4A6CF7", bg: "#F1F5FF", label: "Ekip" },
  general: { icon: "Â·", color: "#64748B", bg: "#F8FAFC", label: "Genel" },
};
...
export function TodoPage({state,setState,currentUser}) {
  ...
  return <div style={{padding:"clamp(20px,4vw,32px)",flex:1,overflow:"auto",maxWidth:1200,width:"100%",margin:"0 auto"}}>
    ...
    <div style={{background:"#fff",border:"1.5px solid #E2E8F0",borderRadius:15,padding:17,marginBottom:16,boxShadow:"0 6px 20px #0f172a0a"}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:11}}><Field label="MÃ¼ÅŸteri / Proje"><select style={iStyle} value={form.projectId} onChange={e=>{const p=state.projects.find(x=>x.id===e.target.value);setForm({...form,projectId:e.target.value,customer:p?.name||form.customer});}}><option value="">Proje seÃ§in veya mÃ¼ÅŸteri yazÄ±n</option>{state.projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field><Field label="MÃ¼ÅŸteri"><input style={iStyle} value={form.customer} onChange={e=>setForm({...form,customer:e.target.value})} placeholder="MÃ¼ÅŸteri adÄ±"/></Field><Field label="Termin"><input type="date" style={iStyle} value={form.dueDate} onChange={e=>setForm({...form,dueDate:e.target.value})}/></Field></div>
      <Field label="Aksiyon"><input style={iStyle} value={form.action} onChange={e=>setForm({...form,action:e.target.value})} onKeyDown={e=>e.key==="Enter"&&save()} placeholder="YapÄ±lacak aksiyon"/></Field>
      <div style={{display:"flex",justifyContent:"flex-end",gap:7}}>{editingId&&<Btn variant="ghost" onClick={()=>{setEditingId(null);setForm(empty);}}>Ä°ptal</Btn>}<Btn onClick={save}>{editingId?"GÃ¼ncelle":"To-Do Ekle"}</Btn></div>
    </div>
    <div className="todo-columns" style={{display:"grid",gridTemplateColumns:"minmax(0,2fr) minmax(260px,1fr)",gap:15}}><div><div style={{fontSize:11,fontWeight:800,color:"#64748B",marginBottom:8,textTransform:"uppercase"}}>AÃ§Ä±k Aksiyonlar</div>{active.map(t=>{const p=state.projects.find(x=>x.id===t.projectId);const late=t.dueDate&&daysDiff(t.dueDate)>0;return <div key={t.id} style={{background:"#fff",border:`1.5px solid ${late?"#FCA5A5":"#E2E8F0"}`,borderRadius:12,padding:13,marginBottom:8,display:"flex",gap:11,alignItems:"flex-start"}}><input type="checkbox" checked={false} onChange={()=>toggle(t.id)} style={{marginTop:3,accentColor:"#059669"}}/><div style={{flex:1}}><div style={{fontSize:13,fontWeight:800}}>{t.action||t.text}</div><div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:5,fontSize:10}}><span style={{color:p?.color||"#4A6CF7",fontWeight:700}}>{t.customer||p?.name||"Genel"}</span>{t.dueDate&&<span style={{color:late?"#E11D48":"#64748B",fontWeight:late?800:600}}>Termin: {fmt(t.dueDate)}{late?` Â· ${daysDiff(t.dueDate)} gÃ¼n geÃ§ti`:""}</span>}</div></div><button onClick={()=>edit(t)} style={{border:0,background:"transparent",color:"#4A6CF7",cursor:"pointer"}}><Icon name="edit" size={15}/></button><button onClick={()=>confirm("To-Do silinsin mi?")&&remove(t.id)} style={{border:0,background:"transparent",color:"#E11D48",cursor:"pointer"}}><Icon name="trash" size={15}/></button></div>})}{!active.length&&<div style={{padding:35,textAlign:"center",border:"1.5px dashed #CBD5E1",borderRadius:12,color:"#94A3B8"}}>AÃ§Ä±k To-Do yok.</div>}</div><div><div style={{fontSize:11,fontWeight:800,color:"#64748B",marginBottom:8,textTransform:"uppercase"}}>Tamamlananlar</div>{done.map(t=><button key={t.id} onClick={()=>toggle(t.id)} style={{width:"100%",border:"1px solid #E2E8F0",background:"#F8FAFC",borderRadius:9,padding:10,marginBottom:6,textAlign:"left",fontSize:11,color:"#94A3B8",textDecoration:"line-through",cursor:"pointer"}}>{t.action||t.text}</button>)}{!done.length&&<div style={{fontSize:11,color:"#CBD5E1"}}>Tamamlanan kayÄ±t yok.</div>}</div></div>
  </div>;
}
...
export function DeadlinePage({warnings,people,onOpenTask,onOpenTodos}) {
  ...
  return <div style={{padding:"clamp(18px,4vw,28px)",flex:1,overflow:"auto"}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap",marginBottom:18}}><div><h2 style={{margin:0,fontSize:20,display:"flex",alignItems:"center",gap:8}}><Icon name="clock" size={21}/>Termin UyarÄ±larÄ±</h2><p style={{margin:"4px 0 0",fontSize:12,color:"#64748B"}}>{warnings.length} geciken gÃ¶rev</p></div><div style={{display:"flex",gap:5}}>{[["all","TÃ¼mÃ¼"],["critical","Kritik"],["normal","Geciken"]].map(([id,label])=><button key={id} onClick={()=>setFilter(id)} style={{border:0,borderRadius:8,padding:"7px 11px",cursor:"pointer",fontSize:11,fontWeight:700,background:filter===id?"#4A6CF7":"#F1F5FF",color:filter===id?"#fff":"#64748B"}}>{label}</button>)}</div></div>
    <div style={{display:"flex",flexDirection:"column",gap:8}}>{shown.map(t=>{const person=people.find(p=>p.id===t.assignee);return <button key={`${t.projectId||"todo"}-${t.id}`} onClick={()=>t.kind==="todo"?onOpenTodos():onOpenTask(t.id)} style={{border:`1.5px solid ${t.level==="critical"?"#FCA5A5":"#FED7AA"}`,borderRadius:12,background:"#fff",padding:13,display:"flex",alignItems:"center",gap:12,cursor:"pointer",textAlign:"left"}}><div style={{width:42,height:42,borderRadius:11,background:t.level==="critical"?"#FFF1F2":"#FFF7ED",display:"grid",placeItems:"center",color:t.level==="critical"?"#E11D48":"#EA6C00",fontWeight:850,fontSize:12}}>{t.days}g</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:800}}>{t.title}</div><div style={{fontSize:11,color:"#64748B",marginTop:3}}><span style={{color:t.kind==="todo"?"#DB2777":t.projectColor,fontWeight:800}}>{t.kind==="todo"?"TO-DO":t.projectName}</span>{person?` Â· ${person.name}`:""} Â· Termin {fmt(t.dueDate)}</div></div>{t.kind==="todo"?<span style={{background:"#FDF2F8",color:"#DB2777",borderRadius:8,padding:"4px 8px",fontSize:10,fontWeight:800}}>TO-DO</span>:<Badge label={t.status}/>}</button>})}{!shown.length&&<div style={{padding:40,textAlign:"center",color:"#94A3B8",border:"1.5px dashed #CBD5E1",borderRadius:13}}>Bu filtrede termin uyarÄ±sÄ± yok.</div>}</div>
  </div>;
}
...
export function PersonDetailModal({ person, projects, personalTasks, onClose }) {
  ...
  const Row=({t})=>{const dl=delayLvl(t.dueDate,t.status);return <div style={{ background:"#F8FAFC", borderRadius:8, padding:"9px 13px", border:"1.5px solid #E2E8F0", display:"flex", gap:9, alignItems:"center", marginBottom:5 }}>
    ...
    <div style={{ flex:1 }}><div style={{ fontSize:12, fontWeight:600, textDecoration:t.status==="Tamamland\u0131"?"line-through":"none", color:t.status==="Tamamland\u0131"?"#94A3B8":"#1E293B" }}>{t.title}</div><div style={{ fontSize:10, color:"#94A3B8" }}>{t.projectName||"Genel"}{t.msName?` â€” ${t.msName}`:""}</div></div>
  ...
  return <Modal title={`${person.name} Detay`} onClose={onClose} wide>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8, marginBottom:18 }}>
      ...
      <Stat label="GecikmiÅŸ" count={delayed} color="#EA6C00" />
    ...
    {all.length===0&&<div style={{ textAlign:"center", color:"#94A3B8", padding:20 }}>GÃ¶rev atanmamis.</div>}
  </Modal>;
}

// â”€â”€â”€ My Tasks + Notes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export function LogPage({ logs, projects }) {
  ...
  return <div style={{ padding:"24px 28px", flex:1, overflow:"auto" }}>
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
      <div><h2 style={{ margin:0, fontSize:20, fontWeight:800 }}>Aktivite GÃ¼nlÃ¼ÄŸÃ¼</h2><p style={{ margin:"3px 0 0", color:"#64748B", fontSize:13 }}>{filtered.length} kayÄ±t</p></div>
    ...
    {days.length===0&&<div style={{ textAlign:"center", padding:40, color:"#94A3B8" }}>KayÄ±t yok.</div>}
    {days.map(day=><div key={day} style={{ marginBottom:24 }}>
      <div style={{ fontSize:11, fontWeight:700, color:"#94A3B8", textTransform:"uppercase", letterSpacing:1.5, marginBottom:10, display:"flex", alignItems:"center", gap:10 }}>
        ...
        <span>{grouped[day].length} iÅŸlem</span>
      </div>
...
// â”€â”€â”€ Milestone Task Panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
