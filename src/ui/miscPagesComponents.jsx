...
export function TodoPage({state,setState,currentUser}) {
  ...
  return <div className="theme-work-page todos-theme-page" style={{padding:"clamp(20px,4vw,32px)",flex:1,overflow:"auto",maxWidth:1200,width:"100%",margin:"0 auto"}}>
    ...
    <div className="glass-form-panel" style={{background:"#fff",border:"1.5px solid var(--border)",borderRadius:15,padding:17,marginBottom:16,boxShadow:"0 6px 20px #0f172a0a"}}>
    ...
    <div className="todo-columns" style={{display:"grid",gridTemplateColumns:"minmax(0,2fr) minmax(260px,1fr)",gap:15}}><div><div style={{fontSize:11,fontWeight:800,color:"var(--muted)",marginBottom:8,textTransform:"uppercase"}}>Açık Aksiyonlar</div>{active.map(t=>{const p=state.projects.find(x=>x.id===t.projectId);const late=t.dueDate&&daysDiff(t.dueDate)>0;return <div key={t.id} style={{background:"#fff",border:`1.5px solid ${late?"var(--danger)":"var(--border)"}`,borderRadius:12,padding:13,marginBottom:8,display:"flex",gap:11,alignItems:"flex-start"}}><input type="checkbox" checked={false} onChange={()=>toggle(t.id)} style={{marginTop:3,accentColor:"var(--success)"}}/><div style={{flex:1}}><div style={{fontSize:13,fontWeight:800}}>{t.action||t.text}</div><div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:5,fontSize:10}}><span style={{color:p?.color||"var(--accent)",fontWeight:700}}>{t.customer||p?.name||"Genel"}</span>{t.dueDate&&<span style={{color:late?"var(--danger)":"var(--muted)",fontWeight:late?800:600}}>Termin: {fmt(t.dueDate)}{late?` · ${daysDiff(t.dueDate)} gün geçti`:""}</span>}</div></div><button onClick={()=>edit(t)} style={{border:0,background:"transparent",color:"var(--accent)",cursor:"pointer"}}><Icon name="edit" size={15}/></button><button onClick={()=>confirm("To-Do silinsin mi?")&&remove(t.id)} style={{border:0,background:"transparent",color:"var(--danger)",cursor:"pointer"}}><Icon name="trash" size={15}/></button></div>})}{!active.length&&<div style={{padding:35,textAlign:"center",border:"1.5px dashed var(--muted)",borderRadius:12,color:"var(--muted)"}}>Açık To-Do yok.</div>}</div><div><div style={{fontSize:11,fontWeight:800,color:"var(--muted)",marginBottom:8,textTransform:"uppercase"}}>Tamamlananlar</div>{done.map(t=><button key={t.id} onClick={()=>toggle(t.id)} style={{width:"100%",border:"1px solid var(--border)",background:"var(--surface-soft)",borderRadius:9,padding:10,marginBottom:6,textAlign:"left",fontSize:11,color:"var(--muted)",textDecoration:"line-through",cursor:"pointer"}}>{t.action||t.text}</button>)}{!done.length&&<div style={{fontSize:11,color:"var(--muted)"}}>Tamamlanan kayıt yok.</div>}</div></div>
  </div>;
}
...
export function DeadlinePage({warnings,people,onOpenTask,onOpenTodos}) {
  ...
  return <div className="theme-work-page deadlines-theme-page" style={{padding:"clamp(18px,4vw,28px)",flex:1,overflow:"auto"}}>
    <div className="unified-page-header deadlines-page-header" style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap",marginBottom:18}}><div><h2 style={{margin:0,fontSize:20,display:"flex",alignItems:"center",gap:8}}><Icon name="clock" size={21}/>Termin Uyarıları</h2><p style={{margin:"4px 0 0",fontSize:12,color:"var(--muted)"}}>{warnings.length} geciken görev</p></div><div className="unified-filter-row" style={{display:"flex",gap:5}}>{[["all","Tümü"],["critical","Kritik"],["normal","Geciken"]].map(([id,label])=><button key={id} onClick={()=>setFilter(id)} style={{border:0,borderRadius:8,padding:"7px 11px",cursor:"pointer",fontSize:11,fontWeight:700,background:filter===id?"var(--accent)":"var(--accent-ink)",color:filter===id?"#fff":"var(--muted)"}}>{label}</button>)}</div></div>
    <div style={{display:"flex",flexDirection:"column",gap:8}}>{shown.map(t=>{const person=people.find(p=>p.id===t.assignee);return <button key={`${t.projectId||"todo"}-${t.id}`} onClick={()=>t.kind==="todo"?onOpenTodos():onOpenTask(t.id)} style={{border:`1.5px solid ${t.level==="critical"?"var(--danger)":"color-mix(in srgb, var(--warning) 24%, var(--border))"}`,borderRadius:12,background:"#fff",padding:13,display:"flex",alignItems:"center",gap:12,cursor:"pointer",textAlign:"left"}}><div style={{width:42,height:42,borderRadius:11,background:t.level==="critical"?"color-mix(in srgb, var(--danger) 9%, white 91%)":"color-mix(in srgb, var(--warning) 10%, white 90%)",display:"grid",placeItems:"center",color:t.level==="critical"?"var(--danger)":"var(--warning)",fontWeight:850,fontSize:12}}>{t.days}g</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:800}}>{t.title}</div><div style={{fontSize:11,color:"var(--muted)",marginTop:3}}><span style={{color:t.kind==="todo"?"#DB2777":t.projectColor,fontWeight:800}}>{t.kind==="todo"?"TO-DO":t.projectName}</span>{person?` · ${person.name}`:""} · Termin {fmt(t.dueDate)}</div></div>{t.kind==="todo"?<span style={{background:"#FDF2F8",color:"#DB2777",borderRadius:8,padding:"4px 8px",fontSize:10,fontWeight:800}}>TO-DO</span>:<Badge label={t.status}/>}</button>})}{!shown.length&&<div style={{padding:40,textAlign:"center",color:"var(--muted)",border:"1.5px dashed var(--muted)",borderRadius:13}}>Bu filtrede termin uyarısı yok.</div>}</div>
  </div>;
}
    ...
    {days.map(day=><div key={day} style={{ marginBottom:24 }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
        {grouped[day].map(l=>{
          ...
          return <div key={l.id} style={{ background:"#fff", borderRadius:12, padding:"12px 16px", border:"1.5px solid var(--border)", display:"flex", gap:12, alignItems:"flex-start", boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", marginBottom:3 }}>
                ...
                {l.project&&<span style={{ background:"var(--accent-ink)", color:"var(--accent)", borderRadius:8, padding:"2px 8px", fontSize:11, fontWeight:600 }}>{l.project}</span>}
                {l.milestone&&<span style={{ background:"var(--surface-soft)", color:"var(--accent)", borderRadius:8, padding:"2px 8px", fontSize:11 }}>{l.milestone}</span>}
              </div>
...
export function NotificationsPage({ notifications, currentUser, setState, onOpenTask, dateChangeRequests = [], onResolveDateChange }) {
  ...
  return <div style={{ padding:"24px 28px", flex:1, overflow:"auto" }}>
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
      ...
      {mine.length>0&&<button onClick={()=>setState(s=>({...s,notifications:(s.notifications||[]).filter(n=>!isNotificationForUser(n,currentUser))}))} style={{ background:"none", border:"none", cursor:"pointer", color:"var(--muted)", fontSize:12 }}>Tümünü Temizle</button>}
    </div>
    {mine.length===0&&<div style={{ textAlign:"center", padding:"50px", background:"#fff", borderRadius:16, border:"1.5px dashed var(--border)" }}>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
      {mine.map(n=>{const request=dateChangeRequests.find(item=>item.id===n.requestId);return <div key={n.id} style={{ background:"#fff", borderRadius:12, padding:"14px 18px", border:`1.5px solid ${n.read?"var(--border)":"var(--accent)"}`, display:"flex", gap:12, alignItems:"flex-start", boxShadow:n.read?"none":"0 2px 8px rgba(74,108,247,0.1)", cursor:n.taskId?"pointer":"default" }} onClick={()=>{markRead(n.id);if(n.taskId)onOpenTask?.(n.taskId);}}>
        <div style={{ width:36, height:36, borderRadius:"50%", background:n.read?"var(--accent-ink)":"var(--accent-ink)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>📋</div>
        <div style={{ flex:1 }}>
        ...
        <button onClick={e=>{e.stopPropagation();deleteNotif(n.id);}} style={{ background:"none",border:"none",cursor:"pointer",color:"var(--muted)",fontSize:16,padding:0 }}>×</button>
      </div>})}
    </div>
  </div>;
}
