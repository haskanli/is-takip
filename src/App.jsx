...
export default function App() {
  ...
  return <><GlobalStyle /><div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} style={{ display:"flex", height:"100vh", width:"100vw", fontFamily:"var(--font-ui, Manrope, Segoe UI, sans-serif)", background:"var(--bg, #f1f5f6)", color:"var(--text, #112327)", overflow:"hidden", position:"relative" }}>
    {/* Mobil ust bar */}
    {isMobile&&<div className="glass-surface" style={{ position:"fixed", top:0, left:0, right:0, height:58, borderTop:0, borderLeft:0, borderRight:0, display:"flex", alignItems:"center", padding:"0 14px", zIndex:900, gap:10 }}>
    ...
    {!isMobile&&<div className="glass-surface" style={{ width:isMobile?220:254, display:"flex", flexDirection:"column", flexShrink:0, borderTop:0, borderBottom:0, borderLeft:0,
      ...
      <div style={{ padding:"20px 16px 15px", borderBottom:"1px solid var(--glass-border, rgb(207 224 227 / 68%))", background:"radial-gradient(circle at top left,rgb(11 138 148 / 12%),transparent 44%),radial-gradient(circle at 90% 12%,rgb(191 122 18 / 10%),transparent 34%)" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <button onClick={()=>navigateTo("dashboard")} style={{border:0,background:"transparent",display:"flex",alignItems:"center",gap:10,cursor:"pointer",minWidth:0}}><img src={tenantProfile.logoUrl||corjectLogo} alt="" style={{width:42,height:42,objectFit:"contain",filter:"drop-shadow(0 7px 14px rgb(11 138 148 / 28%))"}}/><span style={{fontFamily:"var(--font-display, Fraunces, Georgia, serif)",fontSize:20,fontWeight:800,color:"var(--text, #112327)",letterSpacing:"-.025em",lineHeight:1,maxWidth:135,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{tenantProfile.name}</span></button>
