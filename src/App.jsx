...
import { FieldOperationsPage as SharedFieldOperationsPage, FieldVisitModal as SharedFieldVisitModal } from "./ui/fieldOperationsComponents.jsx";
import {
  EmptyMobileRow,
  MobileBottomNav,
...
function ProjectActionsPanel({project,currentUser,state,setState,isAdmin,canManage}) {
  ...
  return <div style={{flex:1,overflow:"auto",padding:"clamp(16px,3vw,26px)",maxWidth:1050,width:"100%",display:"flex",flexDirection:"column"}}>
    ...
    {visitPlan&&<SharedFieldVisitModal plan={visitPlan} project={project} currentUser={currentUser} onClose={()=>setVisitPlan(null)} onSave={data=>{setState(s=>({...s,fieldPlans:(s.fieldPlans||[]).map(plan=>plan.id===visitPlan.id?{...plan,...data,status:"completed",completedAt:plan.completedAt||now(),updatedAt:now()}:plan)}));setVisitPlan(null);}}/>}
  </div>;
}
